...
from common.job_state import JobTypeDef, _row_to_dict
...
def mock_db():
    ...
    with patch("common.job_registry._get_conn") as mock_conn_fn:
        mock_conn = MagicMock()
...
def mock_scheduler():
    ...
    with patch("common.job_registry.make_scheduler") as mock_make:
        scheduler = MagicMock()
...
class TestScheduleRecurring:
    ...
    def test_schedule_cron(self, mock_db, mock_scheduler):
        from common.job_registry import JobManager
        with patch("common.job_registry.make_trigger") as mock_trigger:
