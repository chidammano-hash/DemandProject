...
class TestUpdateGridWithPredictions:
    ...
    def test_updates_qty_for_target_month(self, masked_grid):
        ...
        preds = pd.DataFrame({"dfu_ck": ["A", "B"], "basefcst_pref": [111.0, 222.0]})
        updated = update_grid_with_predictions(masked_grid, month, preds)
        a_row = updated[(updated["dfu_ck"] == "A") & (updated["startdate"] == month)]
        b_row = updated[(updated["dfu_ck"] == "B") & (updated["startdate"] == month)]
    ...
    def test_lag1_for_next_month_equals_prediction(self, masked_grid):
        ...
        preds = pd.DataFrame({"dfu_ck": ["A", "B"], "basefcst_pref": [123.0, 456.0]})
        updated = update_grid_with_predictions(masked_grid, month, preds)
        a_next = updated[(updated["dfu_ck"] == "A") & (updated["startdate"] == next_month)]
        b_next = updated[(updated["dfu_ck"] == "B") & (updated["startdate"] == next_month)]
    ...
    def test_lag1_was_zero_before_update(self, masked_grid):
        ...
        next_month = pd.Timestamp("2024-04-01")
        a_before = masked_grid[(masked_grid["dfu_ck"] == "A") & (masked_grid["startdate"] == next_month)]
    ...
    def test_rolling_mean_recomputed(self, masked_grid):
        ...
        month = pd.Timestamp("2024-03-01")
        preds = pd.DataFrame({"dfu_ck": ["A", "B"], "basefcst_pref": [500.0, 500.0]})
        before_val = masked_grid[
            (masked_grid["dfu_ck"] == "A") & (masked_grid["startdate"] == month)
        ]["rolling_mean_3m"].iloc[0]
        updated = update_grid_with_predictions(masked_grid, month, preds)
        after_val = updated[
            (updated["dfu_ck"] == "A") & (updated["startdate"] == pd.Timestamp("2024-04-01"))
        ]["rolling_mean_3m"].iloc[0]
