...
async def test_update_insight_status_acknowledge():
    ...
    pool, conn, cursor = make_pool(fetchone_return=(
...
async def test_update_insight_status_invalid():
    ...
    pool, conn, cursor = make_pool(description=[])
    with patch("api.core._get_pool", return_value=pool):
        from api.main import app
        transport = ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_update_insight_status_not_found():
    """Row not found → 404."""
    pool, conn, cursor = make_pool(fetchone_return=None, description=[])
...
async def test_auto_accept_dry_run():
    ...
    pool, conn, cursor = make_pool(fetchall_return=rows, description=[])
    with patch("api.core._get_pool", return_value=pool):
        from api.main import app
        transport = ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_auto_accept_empty_results():
    ...
    pool, conn, cursor = make_pool(fetchall_return=[], description=[])
    with patch("api.core._get_pool", return_value=pool):
        from api.main import app
        transport = ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
