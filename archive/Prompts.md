...
Execution lag is the lag where we are interested in reporitng the forecast accuracy at for example items wiht less than 30 day lead time will be having executoon lag 0 and less than 60 2 and 90 3 and so on. so a single DFU can have have only one execution lag. The external forecasts comes only with execution lag but te bactesting forecasts will have forecasts recorded at 0,1,2,3,4 ( 5 lags) out of which only one lag per dfu will execution lag. the accuracy measured at that execution lag is exec lag but for LGBM etc we need the accuracy measured at exec lag and 0,1,2,3,4. Now with this knowledge can you builde the spec for fixign tthe lag filtes for champioon and algoruthms in the model tuning.

we generate at 10 timeframes - which means that you already will be generating the Feb 2026 data 5 times lag 0 will be generated by TimeFrame J , 1 by timeframe I, 2 by timeframe H, 3 by timeframe G and 4 by  timeframe H

oK. create a quick script that will do backtesting for the entire population not just location and create 0,1,2,3,4 lag for the months that are possible with the 10 timeframes and record the accuracy KPIs at 0,1,2,3,4 and at the execution lag for the each DFU. this script takes a config on the algorithm assignement for each archetype. divide the skus into archetypes and assign that algorithm. the default being.  nbeats for erratic low and high, chronos for insufficient and smooth low and mstl for smooth_high. You don't need do any other algorithm. load the forecasts at all lags in the the database with algorithm name ExpSys. Also laod the executive lag forecast in the correct table. Cretae the sspec first and put it and then develop the code.

 I need a PRD and  a spec to load the historical sales at the customer location item month level. We already have sales history at loc and item  and month level. The qty shipped and qty ordered. qty_shipped is the quantity sold or sales_quantity and qty_ordered is the actual demand quantity. this file should be loaded in a sperate table. the volume is high. the file has site the location can be inferred from locationdata.csv or the dim_loc table we need the locatio_id in the table as well. customer_no can be connected to dim_customer and item_no with dim_item. posting period is int the form YYYYMM intthe mpnthly bucket. dim_time has it. demand_qty is the demand quantity or ordered qty, if negative should be zero. sale qty should be demand_cases - oos_cases. we need to load the sales qty sperately as well. I have the files seperated by year but infuture I will be providing onlby month so should be able to load per month partitiion table to drop and load everymonth. excpet for the first load. site,warehouse_no,item_no,customer_no,posting_prd,demand_cases,oos_cases
