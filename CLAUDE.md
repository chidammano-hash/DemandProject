...
| `mvp/demand/frontend/src/tabs/InvPlanningTab.tsx` | Inventory Planning tab: Exception Queue, Portfolio Health, Policy Management, EOQ KPIs, Fill Rate, ABC-XYZ, Supplier, Investment, Intramonth panels (IPfeature4-14) |
...
| `mvp/demand/common/ai_planner.py` | AI Planning Agent core: `AIPlannerAgent` class, 10 tool functions (9 read-only SQL tools + `create_insight`), agentic loop, system prompt (IPAIfeature1) |
...
- Inventory Planning tab (IPfeature4-14): Exception Queue panel, Portfolio Health panel, Policy Management panel, EOQ KPI cards + sensitivity chart, Fill Rate Analytics panel (FillRatePanel), ABC-XYZ Policy Matrix panel (AbcXyzPanel) with 3×3 heatmap and segment detail, Supplier Performance panel (SupplierPanel), Capital Investment Optimization panel (investment efficient frontier chart), Intramonth Stockout panel (IntramonthPanel); sidebar nav item "Inv. Planning"
...
- **Modular API router architecture:** `api/routers/` contains 17 FastAPI `APIRouter` modules (accuracy, ai_planner, analysis, benchmark, chat, clusters, competition, control_tower, dashboard, domains, fill_rate, intel, inv_backtest, inventory, inv_planning, jobs, shap). `main.py` is a ~65-line shell that only creates the app, adds middleware, and mounts all 17 routers via `app.include_router()`. All route handlers live in router modules — no inline routes in main.py. `domains.py` is mounted last because it has catch-all `{domain}` path parameters. All mutation endpoints require `require_api_key` auth when `API_KEY` env var is set.
...
- **AI Planning Agent (IPAIfeature1):** NOT a chatbot — a proactive exception work-queue. `AIPlannerAgent` in `common/ai_planner.py` uses `anthropic.Anthropic()` client with `tool_use` API (model: `claude-opus-4-6`). 10 tools: `get_dfu_full_context`, `get_forecast_performance`, `get_portfolio_exceptions`, `compute_bias_trend`, `get_inventory_trend`, `get_eoq_context`, `get_similar_dfus`, `check_stockout_history`, `get_portfolio_health_summary` (all read-only SQL), and `create_insight` (INSERT into `ai_insights`). Agentic loop: call Claude → dispatch tool_use blocks → append results → repeat until `stop_reason == "end_turn"`. Three async methods: `run_dfu_analysis(item_no, loc, scan_run_id)`, `run_portfolio_scan(scan_run_id)`, `generate_portfolio_memo(period, scan_run_id)`. POST /ai-planner/portfolio-scan returns 202 immediately; scan runs in background thread via `_executor.submit(...)`. GET /ai-planner/insights uses `GET /ai-planner/insights?severity=&status=&insight_type=&page=&page_size=`. Config in `config/ai_planner_config.yaml` — model, thresholds, schedule. Dependency: `anthropic>=0.40.0`.
...
10. **Run `make test-all`** — Verify all 1109+ tests pass (both backend and frontend) before considering the work complete
