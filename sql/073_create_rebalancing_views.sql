...
SELECT
    ...
    AVG(CASE WHEN a.avg_daily_sales > 0
        THEN a.eom_qty_on_hand / a.avg_daily_sales ELSE NULL END)            AS avg_dos,
    STDDEV(CASE WHEN a.avg_daily_sales > 0
        THEN a.eom_qty_on_hand / a.avg_daily_sales ELSE NULL END)            AS stddev_dos,
    CASE WHEN AVG(CASE WHEN a.avg_daily_sales > 0
        THEN a.eom_qty_on_hand / a.avg_daily_sales ELSE NULL END) > 0
    THEN STDDEV(CASE WHEN a.avg_daily_sales > 0
        THEN a.eom_qty_on_hand / a.avg_daily_sales ELSE NULL END)
        / AVG(CASE WHEN a.avg_daily_sales > 0
            THEN a.eom_qty_on_hand / a.avg_daily_sales ELSE NULL END)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_netbal_item ON mv_network_balance (item_no);
