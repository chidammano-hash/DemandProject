-- Add multi-factor stockout risk score (0-100) to integrated planning targets
-- Combines 4 factors: DOS vs lead time, SS coverage, demand variability, ABC class
-- Higher score = more urgent stockout risk
...
SELECT
    ...
    LEAST(100, GREATEST(0,
        -- Factor 1: Days of supply vs lead time (0-40 pts)
        -- If DOS <= 0, max risk; if DOS < LT, proportional risk; if DOS < 2*LT, moderate
        CASE
        ...
        +
        -- Factor 2: Safety stock coverage (0-30 pts)
        -- Below SS = high risk; no SS target = moderate risk; zero stock = max
        CASE
        ...
        +
        -- Factor 3: Demand variability (0-20 pts)
        -- High CV = less predictable = higher risk
        CASE
        ...
        +
        -- Factor 4: ABC class weight (0-10 pts)
        -- A-class items have more impact when stocking out
        CASE
