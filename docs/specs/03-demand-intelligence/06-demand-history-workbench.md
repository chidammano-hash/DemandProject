...
| fact_customer_forecast | Completed `customer_rule_router_v2` rows with the exact persisted customer-only statistical route for an item-location-customer Workbench overlay; completed v1 rows remain readable as historical evidence |
...
disabled only when the current selection is ambiguous or incomplete. Its label
identifies the persisted MA(3), trailing-average, seasonal-repeat, TSB, ADIDA,
Croston/SBA, SES, or damped-Holt route rather than inferring a method from the
