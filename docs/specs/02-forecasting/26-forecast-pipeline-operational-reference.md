...
| Term | Definition |
|---|---|
| **DFU** | Demand Forecast Unit. Backtest/training evidence is resolved at `item_id + customer_group + location`; the published production fact is aggregated to `item_id + location`. Production includes an item/location only when every active customer group satisfies the applicable history floor. |
...
| **Cluster Strategy** | How LightGBM is trained: `per_cluster` trains a separate model per demand cluster; `global` trains one model across all DFUs. |
...
| **Foundation Model** | Chronos 2E uses pretrained weights with historical, known-future, and categorical covariates; it does not require a persisted production-training artifact. |
...
| 5 | `make backtest-all` | Multi-hour | Backtest all five canonical models (uses tuned LightGBM params if step 4 ran) |
...
| 8 | `make train-production-all` | Model-dependent | Final-refit the three persisted families: LightGBM, N-BEATS, and N-HiTS |
...
| 5. Clustering | Data Loading (+ optionally 2, 3) | Yes (for per-cluster LightGBM) |
...
- `scripts/etl/normalize_dataset_csv.py` -- unified normalizer (takes `--dataset` flag for item, sales, forecast, sku, etc.)
- `scripts/etl/normalize_inventory_csv.py` -- dedicated inventory normalizer (handles large volumes)
...
**Prerequisites:** Clustering complete (for per-cluster LightGBM). `fact_sales_monthly` populated.
...
- `config/forecasting/forecast_pipeline_config.yaml` -- canonical five-model `algorithms` section:
  - **Tree:** `lgbm_cluster`
  - **Foundation:** `chronos2_enriched`
  - **Statistical:** `mstl`
  - **Deep learning:** `nbeats`, `nhits`
  - Lifecycle flags are `enabled`, `tune`, `backtest`, `compete`, and `forecast`

**Database tables:**

| Table | Read/Write | Purpose |
|---|---|---|
| `current_sku_cluster_assignment` / `dim_sku` | Read | Promoted cluster assignments, execution lag, and attributes |
| `fact_sales_monthly` | Read | Historical actuals for train/test |

**CLI commands:**

The `backtest-all` target runs the complete canonical roster sequentially.

```bash
# Canonical five (all run by backtest-all)
...
# Composite targets
make backtest-all              # all five sequentially
make backtest-all-parallel     # all five concurrently (logs in data/backtest/logs/)
...
**Scripts:**
- `scripts/ml/run_backtest.py` -- LightGBM backtest. Per-cluster training with SHAP feature selection, Tweedie objective for
  intermittent clusters, recursive multi-step prediction.
- `scripts/ml/run_backtest_chronos2_enriched.py` -- the Chronos 2 Enriched foundation-model backtest
...
- `data/backtest/lgbm_cluster/shap_values/` -- LightGBM SHAP feature importance

These are evaluation artifacts, not production final fits. Production must not
load a loose `data/models/lgbm_cluster/<cluster>/model.pkl` or a model pickle
from a backtest. The final-fit stage publishes one complete LightGBM set under
`data/models/lgbm_cluster/production_tree/versions/<artifact_set_id>/` and
atomically points `production_tree/active.json` to it.
...
make backtest-load-bulk        # five canonical models with a single index cycle
...
# Per-algorithm load targets (LightGBM loads via `make backtest-load MODEL=lgbm_cluster`;
# dedicated targets exist for each non-tree model)
...
- `config/forecasting/tuning_templates.yaml` -- curated UI experiment templates
...
| `lgbm_tuning_run` | Write | LightGBM tuning run summary |
...
**Scripts:**
- `scripts/ml/tune_hyperparams.py` -- Optuna Bayesian optimization with walk-forward CV
- `scripts/ml/auto_tune.py` -- strategy-based grid search from `tune_strategies.yaml`
- `scripts/ml/tune_cluster_hyperparams.py` -- Per-cluster Bayesian tuning (runs Optuna independently per `ml_cluster`)

**Per-cluster tuning CLI:**
```bash
make tune-lgbm-clusters       # Per-cluster LGBM tuning
make tune-clusters            # Alias for per-cluster LightGBM tuning
```
...
- `config/forecasting/champion_experiment_templates.yaml` -- **35 strategy templates** organized by category:
  ...
  - Advanced: uncertainty_aware, dynamic_window, regime_adaptive, bayesian_model_avg, error_correcting
...
make champion-select         # Run the governed five-model experiment + atomic promotion
...
**Scripts:**
- `scripts/ml/run_champion_selection.py` -- main champion selection pipeline
- `scripts/ml/run_champion_experiment.py` -- experiment runner (called as subprocess)
- `scripts/ml/simulate_champion_strategies.py` -- diagnostic tool for all strategies
- `scripts/ml/train_meta_learner.py` -- trains Random Forest classifier
...
**Prerequisites:** The immutable `fact_sales_monthly_original` mirror is
populated and synchronized by the latest canonical dual-track sales reload; a
legacy `safe_upsert` audit is not acceptable lineage. The current canonical
five-model backtests are loaded and champion selection is complete. LightGBM
has a complete current active tree
bundle; N-HiTS and N-BEATS have current active neural artifacts. MSTL and
Chronos 2E use direct inference and have no product-specific final-fit artifact.
...
| `dim_sku` / `current_sku_cluster_assignment` | Read | Full-grain attributes and promoted LightGBM cluster assignments |
| `fact_sales_monthly_original` | Read | Required synchronized canonical history through the latest closed month; no current-table fallback |
| `forecast_generation_run` / `fact_production_forecast_staging` | Write | Immutable manifest and purpose-scoped future predictions |
| `fact_production_forecast` | Promotion only | Consumer-facing rows replaced only by the verified promotion transaction |
...
make forecast-snapshot-contenders  # Freeze/generate exactly three contenders
```
...
**Scripts:**
- `scripts/ml/train_production_models.py` -- final-refits the three persisted families; publishes complete immutable versions
- `scripts/forecasting/generate_production_forecasts.py` -- resolves the full eligible population, validates exact artifacts/routes, generates forecasts and confidence intervals, and writes an immutable run to `fact_production_forecast_staging`
- `common/ml/tree_artifacts.py` -- atomically publishes and validates the complete LightGBM active bundle before deserialization

**Error Recovery:**
- A missing cluster, stale lineage, incomplete customer-group cohort, invalid
  route, or partial/non-finite adapter output fails the coherent generation run.
  Fix the source, rerun the required final fit/backtest/champion stage, and
  generate a new run; do not zero-fill, relabel, or publish a partial result.
- Dry run first: `make forecast-generate-dry` previews without writing to DB
- A failed final fit cannot replace the prior active artifact pointer; retrying
  creates and validates a new immutable version before activation
...
3. Click **"New Experiment"**
4. Select a template from 35 strategies, toggle competing models, and configure strategy params
...
### 4.4 Maintaining a Canonical Model

1. Update its `params` and lifecycle flags in `config/forecasting/forecast_pipeline_config.yaml`.
2. Run its existing backtest target and load the resulting predictions.
3. Refresh accuracy MVs and re-run champion selection.
4. Keep the roster fixed to LightGBM, Chronos 2E, MSTL, N-HiTS, and N-BEATS unless a separate product decision changes it.
...
| LightGBM artifact unavailable or stale | Missing/invalid `production_tree/active.json`, incomplete version, checksum failure, or lineage mismatch | Run `make train-production MODEL=lgbm_cluster`; do not copy a backtest or loose cluster pickle into the active tree |
| N-HiTS/N-BEATS artifact unavailable or stale | Missing active neural version or source/config/history mismatch | Run `make train-production-all` (or the specific model training job) against the same latest-closed sales snapshot |
...
- **PipelineBuilderPanel**: Runs the registered lifecycle presets, including the fixed five-model
  governed refresh and immutable forecast-publish sequence.
...
|---|---|---|---|
| `forecast_pipeline_config.yaml` | Master pipeline config | Five models, backtest, tuning, champion, production forecast, clustering, tracking | ALL stages |
...
| `tuning_templates.yaml` | UI experiment templates | Curated per-model experiment starting points | Stage 8 UI |
| `champion_experiment_templates.yaml` | Champion strategy templates | 35 strategies: expanding, rolling, decay, ensemble, meta_learner, bandit, etc. | Stage 9 UI |
...
| `forecast_domain_config.yaml` | Seasonality + variability + bias | seasonality thresholds, variability CV classes, bias correction | Stage 2 |
...
| Table | Purpose | Populated By |
|---|---|---|
| `lgbm_tuning_run` | LightGBM tuning experiment summary | Stage 8 |
...
## Section 9: Verified Production Invariants

### Canonical five and persisted-family boundary

The only base models are LightGBM (`lgbm_cluster`), N-HiTS (`nhits`), N-BEATS
(`nbeats`), MSTL (`mstl`), and Chronos 2E (`chronos2_enriched`). LightGBM,
N-HiTS, and N-BEATS require immutable active final-fit artifacts; MSTL and
Chronos 2E execute directly. The generator records the exact tree artifact-set
id and neural artifact ids on its manifest.

The LightGBM runtime root is
`data/models/lgbm_cluster/production_tree/`: `active.json` identifies one
`versions/<artifact_set_id>/` directory containing `metadata.json`,
`training_metadata.json`, `checksums.json`, and the complete model-file roster.
The loader validates the pointer, exact promoted-cluster set, sales/config/
cluster lineage and all checksums before unpickling any model. A global bundle
is valid only when explicitly declared and contains exactly `global`.

### Population and bounded live-forward archive

Training and generation use the same canonical sales source through the latest
closed month: the populated, synchronized `fact_sales_monthly_original` mirror.
The latest positive audit row must come from the canonical dual-track loader,
its row count must match the mirror, and mirror `MAX(load_ts)` must be at or
after batch `started_at` (not `completed_at`). Repair with
`make normalize-sales && make load-sales`; there is no empty-mirror fallback.
Eligibility is evaluated at full customer-group grain, and an
item/location is publishable only when every active group passes the relevant
history floor. Release replacement archives exactly the outgoing `champion`
plus the three frozen top-ranked contenders for lags 0 through 5; no fourth or
unselected staged model enters that archive.

### Non-tree production parity

Production generation now calls the same MSTL, N-HiTS, N-BEATS, and Chronos 2E adapters used by
backtesting through `common/ml/production_non_tree.py`. A heuristic forecast cannot be labeled as a
canonical model.
...
| Persisted production training | `scripts/ml/train_production_models.py` |
| LightGBM bundle manager | `common/ml/tree_artifacts.py` |
| Neural artifact manager | `common/ml/neural_artifacts.py` |
| Production forecast script | `scripts/forecasting/generate_production_forecasts.py` |
| Champion selection script | `scripts/ml/run_champion_selection.py` |
