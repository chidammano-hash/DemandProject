...
| **Key Files** | `api/routers/forecasting/tuning/` (split from the legacy 1,798-LoC `unified_model_tuning.py` -- see [Router Layout](#router-layout)), `frontend/src/tabs/ModelTuningTab.tsx`, `frontend/src/tabs/model-tuning/`, `frontend/src/api/queries/unified-model-tuning.ts` |
...
3. [Curated Experiment Plans](#curated-experiment-plans)
...
|-----------|-------------|
| **Experiment Builder** | Form-based UI to configure LightGBM hyperparameters from production, curated, or custom starting points |
...
- Curated templates load the maintained Run 2-5 configurations from this spec
...
- Parameters are grouped into collapsible LightGBM sections: **Tree Structure**, **Regularization**, **Sampling**, and **Advanced**.
...
**LightGBM parameter set:** the form exposes only parameters accepted by the canonical LightGBM registry and validates them against the maintained bounds.
...
- **Group**: `tuning` (only one LightGBM tuning job runs at a time)
...
- Shows a LightGBM model-type badge in the type column
...
- `"curated"`: Reads a maintained LightGBM starting point from `config/forecasting/tuning_templates.yaml`
...
def _run_model_tuning_experiment(params, progress_cb, cancel_event, job_id):
    ...
    2. Build subprocess command:
       uv run python scripts/ml/run_backtest.py --model {model} --config {config_path}
...
The `tuning` job group allows **one LightGBM tuning job at a time**:
...
### 12.1 Curated LightGBM Templates

`config/forecasting/tuning_templates.yaml` contains one live-production template and four curated
LightGBM starting points. The production entry uses `source: algorithm_config`, so the API fills it
from `forecast_pipeline_config.yaml`. Curated entries use `source: curated` and stable IDs:

- `curated_conservative`
- `curated_high_precision`
- `curated_intermittent`
- `curated_balanced`

LightGBM validation bounds and lifecycle settings remain in the forecasting configuration; there is
no separate model-tuning config file.
---
...
| `pre-fills params on template select` | Selecting "Conservative (Stable Demand)" fills max_depth=10, num_leaves=63 |
...
**File:** `frontend/src/tabs/__tests__/EnhancedComparisonPanel.test.tsx`
...
**File:** `frontend/src/tabs/__tests__/EnhancedPromoteModal.test.tsx`
...
**File:** `frontend/src/tabs/__tests__/JobsTuningIntegration.test.tsx`
...
### 13.10 Browser and component coverage

`frontend/e2e/tests/navigation.spec.ts` covers semantic navigation to the Model
Tuning workspace. Builder, comparison, lag-filter, promotion, log-viewer, and
Jobs integration flows are covered by the focused Vitest files listed below;
there is no separate `model-tuning.spec.ts` file.
...
6. Add backward-compatible aliases in existing routers
7. Write all backend tests
8. Add `/model-tuning` prefix to `vite.config.ts` proxy
...
### Must Have (P0)
- [ ] User can configure LightGBM hyperparameters in the UI and launch an experiment
...
- [ ] Current-production plus four curated LightGBM templates are available
...
| `config/forecasting/tuning_templates.yaml` | `config/forecasting/` | Curated LightGBM experiment templates |
...
| `frontend/src/tabs/__tests__/EnhancedComparisonPanel.test.tsx` | `frontend/src/tabs/__tests__/` | Comparison tests |
| `frontend/src/tabs/__tests__/EnhancedPromoteModal.test.tsx` | `frontend/src/tabs/__tests__/` | Promote modal tests |
| `frontend/src/tabs/__tests__/LogViewer.test.tsx` | `frontend/src/tabs/__tests__/` | Log viewer tests |
| `frontend/src/tabs/__tests__/JobsTuningIntegration.test.tsx` | `frontend/src/tabs/__tests__/` | Jobs tab tuning integration |
| `frontend/e2e/tests/navigation.spec.ts` | `frontend/e2e/tests/` | Model Tuning navigation coverage |
...
2. **Curated LightGBM experiment plans** — maintained runs with testable hypotheses and rationale
...
1. **Auto-Tune Campaign** (CLI, batch) - runs predefined strategies from
   ...
   `make lgbm-auto-tune RUNS=N` / `make lgbm-auto-tune-promote RUNS=N` for LGBM.
