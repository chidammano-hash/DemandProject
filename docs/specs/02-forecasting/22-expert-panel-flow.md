...
`ml_cluster` is used for **per-cluster model partitioning** only — it is no longer included as a model feature (removed to prevent leakage from full-history cluster assignments). See [spec 23](23-feature-selection-pipeline.md) and [Known Gaps §1](../01-foundation/08-known-gaps.md).
