# Feature 47: Dual Promotion — Parameters + Results
...
The Model Tuning Studio has a single "Promote" action that writes hyperparameters to `algorithm_config.yaml`. But backtest predictions from tuning experiments are never loaded into PostgreSQL, so:
...
- Writes hyperparameters to `config/algorithm_config.yaml`
