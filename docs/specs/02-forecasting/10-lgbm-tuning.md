...
- All hyperparameters from `config/algorithm_config.yaml` (learning_rate, num_leaves, max_depth, etc.)
...
### `config/lgbm_tuning_config.yaml`

```yaml
# LGBM Tuning run management (Feature 45)

tuning:
  # Directory for backing up run artifacts
  backup_dir: data/backtest/tuning_archive

  # Auto-register a tuning run when run_backtest.py starts
  auto_register: true

  # Auto-compare new completed run against the most recent completed run
  auto_compare_to_latest: true

  # Minimum completed runs before auto-compare activates
  min_runs_for_auto_compare: 2

  # Backup artifact retention (number of runs to keep; 0 = keep all)
  max_backups: 20

  # Verdict thresholds
  verdict:
    # Minimum absolute accuracy delta to declare improved/degraded
    # Below this threshold, verdict is "mixed" regardless of direction
    min_delta_pct: 0.1
```
...
| `make lgbm-auto-tune-promote RUNS=5` | Auto-tune + promote best run's params to algorithm_config.yaml |
...
# Run 5 strategies and promote the best params into algorithm_config.yaml
...
This file can be referenced in `algorithm_config.yaml` via `params_file: data/tuning/best_params_lgbm.json`
...
2. Create `config/lgbm_tuning_config.yaml`
...
```bash
# 1. Edit config/algorithm_config.yaml — change the params you want to test
