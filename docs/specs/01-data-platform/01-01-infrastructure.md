...
- `PostgreSQL 16` (via `pgvector/pgvector:pg16`) as the primary data store for all facts, dimensions, and vector embeddings
...
## Why This Works
- `PostgreSQL 16` provides reliable OLTP storage with materialized views for fast analytics, GIN trigram indexes for full-text search, and pgvector for NL query embeddings.
...
1. Normalize raw CSV files into clean CSVs via Python scripts.
2. Load clean CSVs into PostgreSQL via `psycopg` copy/insert scripts.
3. FastAPI serves SQL analytics and ML results to the React UI.
4. App writes business actions (override, approval, comments, audit) to Postgres.
5. ML training/scoring jobs log params, metrics, and artifacts to MLflow.
...
- Partition fact tables by composite keys (item + location + date) with B-tree indexes.
- Apply GIN trigram indexes for full-text substring search on text columns.
- Use materialized views for pre-aggregated analytics (O(1) KPI queries).
...
- ML models: LightGBM, CatBoost, XGBoost with lag, rolling stats, calendar, and promo features.
- Champion/challenger selection by segment using `WMAPE`, bias, and service impact.
- Version each run (`run_id`, model version, training window) in MLflow + Postgres metadata.

## Simple MVP Scope
1. Ingest `actuals` and master dimensions.
2. Normalize and load into PostgreSQL.
3. Train baseline + one ML model.
4. Write forecasts + metrics to Postgres.
5. Expose FastAPI-backed endpoints for portfolio, exceptions, and item drilldown.
6. Add overrides and approvals in Postgres with full audit.

## Implemented Features (MVP)
- **Feature 1:** Infrastructure setup — Docker Compose, MLflow, Postgres, FastAPI, React
...
- **IPAIfeature1:** AI Planning Agent — Claude `tool_use` agent that reads across all data layers (forecast → inventory → EOQ → policy), traces causal chains, and writes structured, ranked planning insights to a proactive exception work-queue. NOT a chatbot — an automated exception scanner. Agent architecture: `common/ai_planner.py` with `AIPlannerAgent` class and 10 tools (9 read-only PostgreSQL tools + 1 `create_insight` write tool), `api/routers/ai_planner.py` (5 endpoints: POST /ai-planner/analyze, POST /ai-planner/portfolio-scan, GET /ai-planner/insights, PUT /ai-planner/insights/{id}/status, GET /ai-planner/memos). DDL: `sql/026_create_ai_insights.sql` — `ai_insights` + `ai_planning_memos` tables. Config: `config/ai_planner_config.yaml` (model: claude-opus-4-6, thresholds, schedule). Script: `scripts/generate_ai_insights.py` (portfolio scan or single DFU, `--dry-run`). Frontend tab: `frontend/src/tabs/AIPlannerTab.tsx` — portfolio health bar (4 KPI chips), insight cards (severity badge, causal chain reasoning, financial impact, Acknowledge/Resolve buttons), planning memo panel (latest markdown narrative + previous memos). New types: `frontend/src/types/ai_planner.ts`. New UI component: `frontend/src/components/ui/select.tsx` (minimal shadcn/ui Select API wrapper). API query functions added to `frontend/src/api/queries.ts`. "AI Planner" nav item added to `AppSidebar.tsx` (14 items total). "aiPlanner" tab added to `useUrlState.ts` VALID_TABS (12 total). Lazy import + tab case added to `App.tsx`. Vite proxy entry `/ai-planner` added. Make targets: `ai-insights-schema`, `ai-insights-scan`, `ai-insights-dfu`, `ai-insights-all`. Dependency: `anthropic>=0.40.0`. Tests: 18 backend unit (`test_ai_planner.py`), 10 API (`test_ai_planner_api.py`), 7 frontend (`AIPlannerTab.test.tsx`). Backend: 879 passed. Frontend: 265 passed.

- **F1.1 (feature_06_01):** Production Forecast Generation Pipeline — end-to-end inference pipeline that loads persisted champion cluster models, applies per-DFU champion assignments, builds inference feature grids, runs recursive multi-step scoring, and writes versioned production forecasts to `fact_production_forecast`. New DDL: `sql/039_create_production_forecast.sql` — `fact_production_forecast` (grain = item_no + loc + plan_version + month_start, includes CI bands) + `fact_model_registry` tables. Config: `config/production_forecast_config.yaml` (inference settings, model_selection, plan_version format, retention policy, scheduler config). Script: `scripts/generate_production_forecasts.py` — `load_active_models()`, `get_champion_assignments()`, `build_inference_grid()`, `generate_forecast_recursive()`, `write_forecast()`, `purge_old_versions()`, argparse CLI with `--dry-run`, `--item`, `--loc`. API router: `api/routers/production_forecast.py` — 3 endpoints (`GET /forecast/production`, `GET /forecast/production/summary`, `GET /forecast/production/versions`). Frontend: `DemandForecastPanel.tsx` in `frontend/src/tabs/inv-planning/` — KPI cards, ABC breakdown bar chart, DFU drill-down line chart with upper/lower CI bands; "Demand Fcst" sub-tab added to `InvPlanningTab.tsx`. Query module: `frontend/src/api/queries/production-forecast.ts` — TypeScript interfaces + `fetchProductionForecast`, `fetchProductionForecastSummary`, `fetchProductionForecastVersions`. `common/backtest_framework.py` extended: `model_persistence_fn` callback added to `run_tree_backtest()` — called after the final timeframe to serialize trained cluster model dicts as `.pkl` files; all three backtest scripts (`run_backtest.py`, `run_backtest_catboost.py`, `run_backtest_xgboost.py`) implement the callback. Job integration: `generate_production_forecast` job type added to `forecast` group in `common/job_registry.py` + `common/job_state.py`. Make targets: `forecast-prod-schema`, `forecast-generate`, `forecast-generate-dfu`, `forecast-generate-dry`, `forecast-prod-all`. Tests: 9 backend unit (`tests/unit/test_production_forecast.py`), 8 API (`tests/api/test_production_forecast.py`), 2 frontend (`InvPlanningTab.test.tsx` updated). Backend: 1109 passed. Frontend: 380 passed.

- **Forecast CI Bands** (`common/forecast_ci.py`): Residual-based P10/P90 confidence interval computation for the production forecast. Per-DFU RMSE is estimated from backtest residuals (`backtest_lag_archive`), scaled by the horizon factor √h to model forecast uncertainty growth over time, and used to derive symmetric CI bounds around the point forecast. Cluster-level RMSE pooling is applied as a fallback when individual DFU history is sparse. CI bands populate `forecast_qty_lower` and `forecast_qty_upper` in `fact_production_forecast` and are consumed downstream by the Forward-Looking Replenishment Plan to derive month-specific demand sigma without requiring a separate simulation step.

- **Per-DFU Interactive SHAP in DFU Analysis Tab**: On-demand SHAP feature contribution visualization triggered by clicking any backtest model forecast line in `OverlayChartPanel`. New endpoint `GET /forecast/shap/{model_id}/dfu?item_no=&loc=&top_n=` in `api/routers/shap.py` loads persisted pkl from `data/models/{model_id}/cluster_{ml_cluster}.pkl`, rebuilds the exact feature matrix (lags 1–12, rolling mean/std with ddof=1, calendar, categoricals encoded via `_detect_model_framework()`, item numerics), runs SHAP (CatBoost native `ShapValues`, LGBM/XGBoost via `shap.TreeExplainer`), and returns per-month signed contributions for historical + future production-forecast months. New `DfuShapPanel.tsx` component renders a stacked Recharts BarChart below the overlay chart; future months at 45% opacity; 15-color palette per feature; scrollable container; fallback to cluster-level summary on 404. `DfuAnalysisTab.tsx` manages `selectedModel` state, resets on DFU change, and only renders `DfuShapPanel` in `item_location` mode. `DfuShapPanel.tsx` regex-extracts raw model_id from formatted "Production Forecast (model_id)" label before calling `fetchDfuShap`. New query function `fetchDfuShap` in `frontend/src/api/queries/core.ts`. New TypeScript types in `frontend/src/types/shap.ts` (`DfuShapFeatureContribution`, `DfuShapPoint`, `DfuShapPayload`). Tests: 6 backend API (`tests/api/test_shap_dfu.py`), 5 frontend (`DfuShapPanel.test.tsx`). Bug fix: `generate_production_forecasts.py` rolling std now uses `ddof=1` at all 3 inference locations to match pandas `rolling.std()` used during training — eliminates systematic ~18% feature discrepancy for 3-month window.

- **InvPlanningTab Grouped Sidebar Navigation (PL-009 Extension):** Redesigned `InvPlanningTab.tsx` from a flat `flex-wrap` 26-tab strip to a two-column enterprise layout. Left sidebar (220px, fixed) contains 7 color-coded navigation groups (Daily Operations/red, Optimize/blue, Analytics/emerald, Planning/violet, Sensing/teal, Strategic/amber, Supply/slate); each group has a labeled divider and icon+label tab buttons with a colored active left-border indicator. Main content area shows a fixed panel header bar (title + description from `PANEL_META`) above the scrollable active panel body. All 26 panel components unchanged. Tests updated: 3 `getByText()` → `getAllByText()` for duplicate panel title text.

- **Forward-Looking Replenishment Plan** (`fact_replenishment_plan`, `scripts/compute_replenishment_plan.py`): Per-DFU per-month forward replenishment parameters driven by the ML production forecast CI bands. Grain = `(plan_version, item_no, loc, plan_month)` — 12 forward months per DFU per plan version. Safety stock is computed from CI-derived demand sigma (`sigma_demand_monthly = (CI_upper - CI_lower) / (2 × 1.282)`) using the same combined formula as IPfeature3 (demand + lead time variance), with historical `demand_std` as fallback when CI bands are absent. EOQ and cycle stock are computed from `forecast_annual_demand` (12-month forward sum). Policy parameters (ROP, order quantity, order-up-to level) are derived per DFU from `fact_dfu_policy_assignment` → `dim_replenishment_policy`, covering all four policy types (continuous_rop, min_max, periodic_review, manual/JIT). Each row carries a comparison vs `fact_safety_stock_targets` (`ss_delta`, `ss_delta_pct`) and current inventory position (`ss_gap`, `is_below_ss`). New DDL: `sql/041_create_replenishment_plan.sql`. Config: `config/replenishment_plan_config.yaml` (CI Z-score, guard rails, EOQ params, retention). New API router: `api/routers/inv_planning_replenishment_plan.py` — 4 endpoints (`GET /inv-planning/replenishment/summary`, `/detail`, `/comparison`, `/dfu`). Frontend: `ReplenishmentPlanPanel.tsx` — plan version selector, 4 KPI cards, CI coverage badge, forward vs historical SS bar chart by ABC class, top-changes table, virtualized detail table, DFU drill-down line chart with CI band shading. Make targets: `replen-plan-schema`, `replen-plan-compute`, `replen-plan-dfu`, `replen-plan-dry`, `replen-plan-all`. Tests: 25+ backend unit (`tests/unit/test_replenishment_plan.py`), 15+ API (`tests/api/test_inv_planning_replenishment_plan.py`), 8+ frontend (`ReplenishmentPlanPanel.test.tsx`). Design spec: `docs/specs/03-inventory-planning/03-11-replenishment-plan.md`.

- **Inventory Rebalancing** — Cross-location transfer optimization with network topology model (`dim_transfer_lane`), greedy + LP solvers, financial ROI analysis, approval workflow, 12 API endpoints, RebalancingPanel UI (27th sub-panel in Inventory Planning).

- **E2E Testing (Playwright):** 8 Playwright E2E smoke test suites covering navigation, dashboard, accuracy, global filters, inventory planning, AI planner, control tower, and theme switching. Playwright config at `frontend/e2e/playwright.config.ts`, shared fixtures at `frontend/e2e/fixtures/base.ts`. DevDependency: `@playwright/test`. npm scripts: `e2e`, `e2e:ui`, `e2e:headed`, `e2e:report`. Makefile targets: `e2e-install`, `e2e`, `e2e-ui`, `e2e-headed`, `e2e-report`. Design spec: `docs/specs/06-ui-platform/06-07-e2e-visual-testing.md`.

- **08-01: Data Quality & Pipeline Observability** — End-to-end data quality monitoring and pipeline observability infrastructure. New DDL: `sql/042_create_data_quality.sql` — `fact_dq_checks`, `fact_dq_results`, `dim_dq_rules` tables with indexes. Config: `config/data_quality_config.yaml` (check definitions, thresholds, schedules). Common module: `common/data_quality.py` — `DQEngine` class with rule evaluation, anomaly detection, freshness checks, and lineage tracking. Script: `scripts/run_data_quality_checks.py` (full pipeline scan, `--dry-run`, `--dataset`). Router: `api/routers/data_quality.py` — endpoints for DQ results, rule CRUD, pipeline status, lineage DAG. Frontend: `DataQualityPanel` in pipeline observability dashboard. Make targets: `dq-schema`, `dq-run`, `dq-all`.

- **08-02: User Management & RBAC** — Role-based access control with user authentication and permission management. New DDL: `sql/043_create_user_management.sql` — `dim_user`, `dim_role`, `fact_user_role_assignment`, `fact_permission` tables with indexes. Config: `config/rbac_config.yaml` (default roles, permission matrix, session settings). Common module: `common/auth_rbac.py` — `RBACManager` class with role hierarchy, permission checks, session management, and audit logging. Router: `api/routers/user_management.py` — endpoints for user CRUD, role assignment, permission checks, login/logout, session management. Frontend: `UserManagementPanel` with role editor, permission matrix, and user directory. Make targets: `rbac-schema`, `rbac-seed`, `rbac-all`.

- **08-03: Performance & Caching Infrastructure** — Application-level caching, query optimization, and performance monitoring. Config: `config/cache_config.yaml` (TTL policies per endpoint category, eviction strategy, max memory). Common module: `common/cache.py` — `CacheManager` class with TTL-based in-memory cache, cache invalidation hooks, query result caching, and cache statistics. Common module: `common/query_optimizer.py` — query plan analysis, slow query logging, index recommendation engine. Router: `api/routers/performance.py` — endpoints for cache stats, cache invalidation, slow query report, performance metrics. Make targets: `cache-warm`, `cache-clear`, `perf-report`.

- **08-04: Notifications & Alerting** — Multi-channel notification delivery for exceptions, job completions, and threshold breaches. New DDL: `sql/044_create_notifications.sql` — `fact_notifications`, `dim_notification_channels`, `fact_notification_preferences` tables with indexes. Config: `config/notification_config.yaml` (channel settings for Slack/Email/PagerDuty, severity routing rules, quiet hours, escalation policies). Common module: `common/notifications.py` — `NotificationEngine` class with channel adapters (Slack webhook, SMTP email, PagerDuty events API), template rendering, delivery tracking, and retry logic. Router: `api/routers/notifications.py` — endpoints for notification history, channel CRUD, preference management, test delivery. Make targets: `notify-schema`, `notify-test`, `notify-all`.

- **08-05: Collaboration & Annotations** — In-context annotations, comments, and collaborative decision tracking on forecasts, exceptions, and inventory items. New DDL: `sql/045_create_collaboration.sql` — `fact_annotations`, `fact_comments`, `fact_decision_log` tables with indexes. Config: `config/collaboration_config.yaml` (annotation types, mention notifications, retention policy). Common module: `common/collaboration.py` — `CollaborationManager` class with annotation CRUD, comment threading, @mention resolution, and decision audit trail. Router: `api/routers/collaboration.py` — endpoints for annotation CRUD, comment threads, decision log, activity feed. Frontend: `AnnotationOverlay` component for inline forecast/chart annotations, `CommentThread` component for contextual discussions. Make targets: `collab-schema`, `collab-seed`, `collab-all`.

- **08-06: Advanced Demand Signals & Causal Intelligence** — External demand signal ingestion and causal impact analysis for forecast enrichment. New DDL: `sql/046_create_causal_signals.sql` — `fact_external_signals`, `dim_signal_sources`, `fact_causal_impact` tables with indexes. Config: `config/causal_signals_config.yaml` (signal source definitions, lag ranges, correlation thresholds, feature engineering rules). Common module: `common/causal_intelligence.py` — `CausalEngine` class with Granger causality testing, cross-correlation analysis, signal-to-feature transformation, and impact attribution. Script: `scripts/ingest_causal_signals.py` (multi-source signal ingestion, `--source`, `--dry-run`). Router: `api/routers/causal_signals.py` — endpoints for signal summary, causal graph, impact attribution, signal source management. Frontend: `CausalIntelligencePanel` with causal DAG visualization and signal impact heatmap. Make targets: `causal-schema`, `causal-ingest`, `causal-analyze`, `causal-all`.

- **08-07: Forecast Value Tracking & ROI Measurement** — Quantitative measurement of forecast improvement value and business ROI across the forecasting lifecycle. New DDL: `sql/047_create_forecast_value.sql` — `fact_forecast_value_tracking`, `fact_roi_metrics`, `mv_forecast_value_summary` materialized view with indexes. Config: `config/forecast_value_config.yaml` (cost assumptions, service level targets, inventory carrying cost rates, lost sale cost multipliers). Common module: `common/forecast_value.py` — `ForecastValueCalculator` class with FVA (forecast value added) computation, inventory cost impact, service level delta, and ROI aggregation. Script: `scripts/compute_forecast_value.py` (period-over-period value tracking, `--baseline-model`, `--dry-run`). Router: `api/routers/forecast_value.py` — endpoints for value summary, ROI dashboard, model comparison value, trend. Frontend: `ForecastValuePanel` with ROI waterfall chart, model value comparison, and period-over-period trend. Make targets: `fv-schema`, `fv-compute`, `fv-all`.

- **08-08: Reporting & Distribution** — Automated report generation and scheduled distribution of analytics summaries, exception digests, and executive dashboards. New DDL: `sql/048_create_reports.sql` — `fact_report_runs`, `dim_report_templates`, `fact_report_distribution` tables with indexes. Config: `config/reporting_config.yaml` (report templates, distribution lists, schedule definitions, output formats). Common module: `common/reporting.py` — `ReportEngine` class with template rendering (HTML/PDF/Excel), chart snapshot capture, distribution scheduling, and delivery tracking. Script: `scripts/generate_reports.py` (batch report generation, `--template`, `--period`, `--dry-run`). Router: `api/routers/reporting.py` — endpoints for report list, template CRUD, on-demand generation, distribution history, download. Make targets: `report-schema`, `report-generate`, `report-all`.

- **08-09: API Governance & Developer Portal** — API versioning, rate limiting, usage analytics, and self-service developer documentation portal. Config: `config/api_governance_config.yaml` (rate limit tiers, versioning policy, deprecation schedule, API key scopes). Common module: `common/api_governance.py` — `APIGovernor` class with rate limiter (token bucket), request logging, usage analytics, API key scope enforcement, and deprecation warnings middleware. Router: `api/routers/api_governance.py` — endpoints for API usage dashboard, rate limit status, API key management, endpoint catalog. Frontend: `DeveloperPortalPanel` with interactive API explorer, usage charts, and rate limit monitor. Make targets: `api-gov-seed`, `api-gov-report`.

- **08-10: Webhooks & REST API Consumers** — Outbound webhook delivery for event-driven integrations with external systems (ERP, WMS, BI tools). New DDL: `sql/049_create_webhooks.sql` — `dim_webhook_endpoints`, `fact_webhook_deliveries`, `fact_webhook_event_log` tables with indexes. Config: `config/webhook_config.yaml` (event types, retry policy, HMAC signing settings, delivery timeout, max payload size). Common module: `common/webhooks.py` — `WebhookManager` class with event subscription, HMAC-SHA256 payload signing, async delivery with exponential backoff retry, delivery status tracking, and dead letter queue. Router: `api/routers/webhooks.py` — endpoints for webhook endpoint CRUD, event subscription management, delivery history, manual replay, health check. Make targets: `webhook-schema`, `webhook-test`, `webhook-all`.
...
For your requirement, use `PostgreSQL + MLflow` as the core platform, fully local on your MacBook.
This gives the best balance of simplicity, reliability, and model governance for 500M+ record demand forecasting with ML models.
...
29 router modules in `api/routers/`. Mounted via `app.include_router()` in `main.py`. `domains.py` is always mounted last (catch-all `{domain}` path parameter).
