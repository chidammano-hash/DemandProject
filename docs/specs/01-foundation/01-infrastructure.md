...
| Customer-Level Forecasting + Bottom-Up Blend (customer-only `customer_rule_router_v2` with frozen per-series MA(3), trailing average, evidence-gated seasonal repeat, TSB, ADIDA, Croston/SBA, SES, or damped-Holt routes; causal per-origin rerouting in the six-origin backtest; exact completed-load/profile-refresh lineage; 18-month demand-to-sales normalization; paired immutable `customer_bottom_up` `shadow_candidate` for read-only Portfolio/Item Analysis comparison; `customer_bottom_up_blend` as the sole promotable customer-derived release candidate; exact run/backtest/staging lineage; migrations `sql/216`–`sql/219`; API `/customer-forecast/blend/trend`) | [../02-forecasting/35-customer-level-forecasting.md](../02-forecasting/35-customer-level-forecasting.md) | Forecasting |
