...
| Customer-Level Forecasting + Bottom-Up Blend (Croston/SBA-only resumable customer runs; exact completed-load/profile-refresh lineage; 18-month demand-to-sales normalization; six-origin common-cohort backtest; paired immutable `customer_bottom_up` `shadow_candidate` for read-only Portfolio/Item Analysis comparison; `customer_bottom_up_blend` as the sole promotable customer-derived release candidate; exact run/backtest/staging lineage; migrations `sql/216`–`sql/217`; API `/customer-forecast/blend/trend`) | [../02-forecasting/35-customer-level-forecasting.md](../02-forecasting/35-customer-level-forecasting.md) | Forecasting |
