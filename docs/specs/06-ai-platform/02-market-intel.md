...
> AI-powered market briefing that combines Google web search results with gpt-4o-mini narrative synthesis for any item-location pair.
...
A `POST /market-intelligence` endpoint accepts an item number and location, looks up item metadata (description, brand, category) and location state from dimension tables, runs a Google Custom Search API query, and sends the search results to gpt-4o-mini for narrative synthesis into a structured market briefing.
...
5. gpt-4o-mini synthesizes the search snippets into a structured narrative briefing with sections: market trends, competitive landscape, regulatory factors, and demand implications.
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/market-intelligence` | Item-location market briefing from web search + gpt-4o-mini |
...
| `briefing` (response) | string | gpt-4o-mini narrative synthesis |
...
| Dependency | Reason |
|---|---|
| `openai` Python package | gpt-4o-mini API client |
