...
| `--yearly-seasonality` | `auto` | Yearly seasonality |
...
NeuralProphet fits are parallelized via Python `multiprocessing.Pool` (identical to Prophet):

- Default: all CPU cores
- Each worker fits one DFU at a time
- Progress reported every 10,000 DFUs
- `imap_unordered` for maximum throughput
- Worker initialization suppresses PyTorch/Lightning logging
...
**Metadata JSON (`backtest_metadata.json`):** Model ID, strategy, timeframes, neuralprophet_kwargs, accuracy metrics.
...
| File | Purpose |
|------|---------|
| `mvp/demand/scripts/run_backtest_neuralprophet.py` | NeuralProphet backtest script (new) |
