...
50. SKU Chatbot (06-07):
   - Conversational, read-only per-SKU assistant with selectable local runtime: **Claude Agent SDK** (`runtime.provider=claude`, default; optional `agent` extra, lazy-imported) or **Codex CLI** (`runtime.provider=codex`, invokes `codex exec` with a read-only SKU context snapshot)
