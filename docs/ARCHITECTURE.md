...
`/domains`, `/jobs`, `/clustering`, `/forecast`, `/inventory`, `/dashboard`, `/health`, `/sku`, `/sku-features`, `/sku-chat`, `/competition`, `/market-intelligence`, `/inv-planning`, `/fill-rate`, `/control-tower`, `/ai-planner`, `/storyboard`, `/supply`, `/analytics`, `/finance`, `/sop`, `/events`, `/scenarios`, `/auth`, `/users`, `/data-quality`, `/notifications`, `/collaboration`, `/demand-signals`, `/demand-history`, `/fva`, `/reports`, `/webhooks`, `/cache`, `/config`, `/sql-runner`, `/sourcing`, `/purchase-orders`, `/lgbm-tuning`, `/catboost-tuning`, `/xgboost-tuning`, `/cluster-eda`, `/cluster-experiments`, `/champion-experiments`, `/feature-lab`, `/accuracy-budget`, `/model-tuning`, `/expsys`, `/customer-analytics`, `/backtest-management`
...
| `common/ml/backtest_framework.py` | `run_tree_backtest()` orchestrator, timeframe generation, data loading, execution-lag assignment, all-lag expansion, post-processing, model-scoped output saving (`data/backtest/<model_id>/`), feature importance; `_fill_predict_nans()`, `_predict_single_month()`, `recursive` param for recursive multi-step inference (Feature 43); configurable `shap_retrain_threshold` from `forecast_pipeline_config.yaml`. `_last_persistable_timeframe()` targets `.pkl` model persistence at the last timeframe with a non-empty predict window so production models still persist when `embargo_months >= 1` (2026-06-20). `_inject_recursive_noise()` is NaN-safe — the recursive noise scale is computed over finite values only, preventing a `NaN` scale from wiping the `qty_lag_2..N` feature block on every recursive fit (2026-06-20). |
...
50. SKU Chatbot (06-07):
   - Conversational, read-only per-SKU assistant on the **Claude Agent SDK** (`claude-agent-sdk`, optional `agent` extra, lazy-imported)
   - Service package `common/ai/sku_chat/` — `config`, `auth` (auth.mode: auto/api_key/bedrock/vertex), `model_router` (per-turn Haiku/Sonnet/Opus tiering), `sku_data` (7 read-only reads), `tools` (in-process SDK MCP `@tool` server, allow-list `mcp__sku__*`), `agent` (SSE event mapping + wall-clock/turn guardrails), `prompts`, `store` (best-effort persistence)
   ...
   - Config: `config/ai/sku_chat_config.yaml` (auth mode, tier→model map, routing heuristics, guardrails, tool allow-list, system prompt, persistence)
...
The interactive complement to the AI Planning Agent: a conversational, read-only per-SKU assistant on the **Claude Agent SDK** (`claude-agent-sdk`). A planner asks free-form questions about one SKU and gets grounded, cited answers streamed over SSE. Each turn routes to a model tier — Haiku (lookups) / Sonnet (analysis) / Opus (multi-signal "why" diagnosis) — and gathers evidence through 7 read-only in-process MCP tools (`sku_data.py`: profile, sales, forecast, inventory, accuracy, cluster peers, search). Auth is a single `auth.mode` switch (auto/api_key/bedrock/vertex): no API key inside Claude Code, API key/Bedrock/Vertex standalone. Surfaced as a standalone **SKU Chat** tab and a **global, page-aware chat drawer on every tab** (its focus, suggested prompts, and SKU scope adapt to the active page). Best-effort persistence to `sku_chat_session|message|call_log`. It can also **drive the champion-forecast adjuster**: an `apply_champion_adjustment` tool stages a guardrail-validated proposal (reusing `champion_adjust_service.py`), an "AI Adjust" button + in-chat approval card gate it, and `POST /sku-chat/adjustment/{id}` performs the human-approved write via the existing `save_adjustment` (`sku_chat_pending_adjustment`, `sql/197`) — the agent never writes the forecast directly. Router `/sku-chat` (`common/ai/sku_chat/`); the SDK is an optional `agent` extra. See `docs/specs/06-ai-platform/07-sku-chatbot.md`.
