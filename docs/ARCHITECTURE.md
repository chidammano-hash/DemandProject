...
The last two are the **AI Champion** forward adjuster store (spec [02-27](specs/02-forecasting/27-ai-champion-forecast.md)): the repurposed AI planner adjusts the promoted champion forecast forward and writes a new forecast with `model_id='ai_champion'`. Generated by [scripts/forecasting/generate_ai_champion_forecast.py](../scripts/forecasting/generate_ai_champion_forecast.py). Forward-only — no backtest, no accuracy grading.
