...
| G-20 | Customer Bottom-Up Governance | Customer forecasts existed only as isolated demand-grain outputs with no target normalization, historical comparison, or governed release bridge | Reproducible normalized customer blend with common-cohort quality gate, review-only bottom-up shadow, and immutable release lineage | **Closed** — ordered customer rule-router generation (migration 218), migrations 216/217 component/accuracy and shadow evidence, production-spine fallback policy, exact cross-screen run lineage, and matching causal backtest requirement; see ADR-020 | Closed |
