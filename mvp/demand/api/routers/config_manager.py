...
CONFIG_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "algorithm_config": {
        ...
        "description": "Backtest and model training parameters for LightGBM, CatBoost, and XGBoost gradient boosting algorithms. Controls cluster strategy, recursive inference, SHAP feature selection, and per-model hyperparameters.",
        "fields": {
            "backtest.n_timeframes": {"label": "Backtest Timeframes", "description": "Number of expanding-window timeframes for backtesting. More timeframes give more robust accuracy estimates but take longer.", "type": "integer", "min": 1, "max": 30},
            "backtest.output_dir": {"label": "Backtest Output Directory", "description": "Directory for backtest prediction CSV output files.", "type": "text"},
            "algorithms.lgbm.enabled": {"label": "LGBM Enabled", "description": "Enable LightGBM model in backtesting and forecasting pipeline.", "type": "boolean"},
            "algorithms.lgbm.cluster_strategy": {"label": "LGBM Cluster Strategy", "description": "Training approach: 'per_cluster' trains a separate model for each demand cluster (better accuracy for heterogeneous portfolios), 'global' trains one model on all data (faster, simpler).", "type": "select", "options": ["per_cluster", "global"]},
            "algorithms.lgbm.recursive": {"label": "LGBM Recursive Inference", "description": "When true, uses recursive multi-step forecasting (model predicts one step at a time, feeding predictions back as features). More accurate for long horizons but slower.", "type": "boolean"},
            "algorithms.lgbm.shap_select": {"label": "LGBM SHAP Feature Selection", "description": "Enable SHAP-based feature selection during backtesting. Drops low-importance features to reduce overfitting.", "type": "boolean"},
            "algorithms.lgbm.shap_threshold": {"label": "LGBM SHAP Threshold", "description": "Cumulative importance threshold for SHAP feature selection. Features are ranked by importance and selected until this fraction of total importance is reached.", "type": "number", "min": 0.5, "max": 1.0, "step": 0.01},
            "algorithms.lgbm.shap_sample_size": {"label": "LGBM SHAP Sample Size", "description": "Number of training rows sampled for SHAP computation. Larger samples are more accurate but slower.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.lgbm.tune_inline": {"label": "LGBM Inline Tuning", "description": "Run lightweight hyperparameter tuning within each backtest fold instead of using fixed parameters.", "type": "boolean"},
            "algorithms.lgbm.n_estimators": {"label": "LGBM Number of Trees", "description": "Maximum number of boosting rounds (trees). More trees can improve accuracy but increase training time and risk overfitting.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.lgbm.learning_rate": {"label": "LGBM Learning Rate", "description": "Step size shrinkage to prevent overfitting. Lower values require more trees but generally produce better models. Typical range: 0.01–0.1.", "type": "number", "min": 0.001, "max": 1.0, "step": 0.005},
            "algorithms.lgbm.num_leaves": {"label": "LGBM Number of Leaves", "description": "Maximum number of leaves per tree. Controls model complexity. Higher values capture more patterns but risk overfitting.", "type": "integer", "min": 8, "max": 256, "step": 1},
            "algorithms.lgbm.min_child_samples": {"label": "LGBM Min Samples per Leaf", "description": "Minimum number of data points required in a leaf node. Higher values prevent the model from learning overly specific patterns.", "type": "integer", "min": 1, "max": 200, "step": 1},
            "algorithms.catboost.enabled": {"label": "CatBoost Enabled", "description": "Enable CatBoost model in backtesting and forecasting pipeline.", "type": "boolean"},
            "algorithms.catboost.cluster_strategy": {"label": "CatBoost Cluster Strategy", "description": "Training approach for CatBoost: 'per_cluster' or 'global'. Same semantics as LGBM cluster strategy.", "type": "select", "options": ["per_cluster", "global"]},
            "algorithms.catboost.recursive": {"label": "CatBoost Recursive Inference", "description": "Enable recursive multi-step forecasting for CatBoost.", "type": "boolean"},
            "algorithms.catboost.shap_select": {"label": "CatBoost SHAP Feature Selection", "description": "Enable SHAP feature selection for CatBoost backtests.", "type": "boolean"},
            "algorithms.catboost.shap_threshold": {"label": "CatBoost SHAP Threshold", "description": "Cumulative SHAP importance threshold for CatBoost feature selection.", "type": "number", "min": 0.5, "max": 1.0, "step": 0.01},
            "algorithms.catboost.shap_sample_size": {"label": "CatBoost SHAP Sample Size", "description": "Number of training rows sampled for CatBoost SHAP computation.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.catboost.tune_inline": {"label": "CatBoost Inline Tuning", "description": "Run lightweight hyperparameter tuning within each CatBoost backtest fold.", "type": "boolean"},
            "algorithms.catboost.iterations": {"label": "CatBoost Iterations", "description": "Maximum number of boosting iterations. CatBoost equivalent of n_estimators.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.catboost.learning_rate": {"label": "CatBoost Learning Rate", "description": "Learning rate for CatBoost. Lower values need more iterations but generalize better.", "type": "number", "min": 0.001, "max": 1.0, "step": 0.005},
            "algorithms.catboost.depth": {"label": "CatBoost Tree Depth", "description": "Maximum depth of each tree. CatBoost uses symmetric (balanced) trees, so depth 6 means 2^6 = 64 leaves.", "type": "integer", "min": 1, "max": 16, "step": 1},
            "algorithms.catboost.l2_leaf_reg": {"label": "CatBoost L2 Regularization", "description": "L2 regularization coefficient for leaf values. Higher values reduce overfitting. Typical range: 1–10.", "type": "number", "min": 0.0, "max": 100.0, "step": 0.5},
            "algorithms.xgboost.enabled": {"label": "XGBoost Enabled", "description": "Enable XGBoost model in backtesting and forecasting pipeline.", "type": "boolean"},
            "algorithms.xgboost.cluster_strategy": {"label": "XGBoost Cluster Strategy", "description": "Training approach for XGBoost: 'per_cluster' or 'global'.", "type": "select", "options": ["per_cluster", "global"]},
            "algorithms.xgboost.recursive": {"label": "XGBoost Recursive Inference", "description": "Enable recursive multi-step forecasting for XGBoost.", "type": "boolean"},
            "algorithms.xgboost.shap_select": {"label": "XGBoost SHAP Feature Selection", "description": "Enable SHAP feature selection for XGBoost backtests.", "type": "boolean"},
            "algorithms.xgboost.shap_threshold": {"label": "XGBoost SHAP Threshold", "description": "Cumulative SHAP importance threshold for XGBoost feature selection.", "type": "number", "min": 0.5, "max": 1.0, "step": 0.01},
            "algorithms.xgboost.shap_sample_size": {"label": "XGBoost SHAP Sample Size", "description": "Number of training rows sampled for XGBoost SHAP computation.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.xgboost.tune_inline": {"label": "XGBoost Inline Tuning", "description": "Run lightweight hyperparameter tuning within each XGBoost backtest fold.", "type": "boolean"},
            "algorithms.xgboost.n_estimators": {"label": "XGBoost Number of Trees", "description": "Maximum boosting rounds for XGBoost.", "type": "integer", "min": 50, "max": 5000, "step": 50},
            "algorithms.xgboost.learning_rate": {"label": "XGBoost Learning Rate", "description": "Learning rate for XGBoost.", "type": "number", "min": 0.001, "max": 1.0, "step": 0.005},
            "algorithms.xgboost.max_depth": {"label": "XGBoost Max Depth", "description": "Maximum tree depth. Deeper trees are more expressive but prone to overfitting.", "type": "integer", "min": 1, "max": 20, "step": 1},
            "algorithms.xgboost.min_child_weight": {"label": "XGBoost Min Child Weight", "description": "Minimum sum of instance weight in a child. Higher values produce more conservative models.", "type": "integer", "min": 1, "max": 100, "step": 1},
            "algorithms.xgboost.subsample": {"label": "XGBoost Row Subsample", "description": "Fraction of training rows sampled per tree. Values < 1.0 add randomness that reduces overfitting.", "type": "number", "min": 0.1, "max": 1.0, "step": 0.05},
            "algorithms.xgboost.colsample_bytree": {"label": "XGBoost Column Subsample", "description": "Fraction of features sampled per tree. Lower values force the model to use different features across trees.", "type": "number", "min": 0.1, "max": 1.0, "step": 0.05},
        },
    },
    ...
    "model_competition": {
        ...
        "category": "forecasting",
        "description": "Champion model selection strategy. Controls how the best-performing model is selected across algorithms using expanding-window evaluation and meta-learning.",
        "fields": {
            "competition.metric": {"label": "Evaluation Metric", "description": "Primary metric for model comparison: 'accuracy_pct' (higher is better).", "type": "select", "options": ["accuracy_pct", "wape", "mape"]},
            "competition.lag": {"label": "Evaluation Lag", "description": "Which backtest lag to use for comparison: 'execution' uses the execution lag (lag 0).", "type": "select", "options": ["execution", "0", "1", "2", "3", "4"]},
            "competition.min_dfu_rows": {"label": "Min DFU Rows", "description": "Minimum data points per DFU required for valid model comparison.", "type": "integer", "min": 1, "max": 12},
            "competition.fallback_model_id": {"label": "Fallback Model", "description": "Model to use when no champion can be determined for a DFU.", "type": "text"},
            "competition.strategy": {"label": "Selection Strategy", "description": "Method for selecting the champion model: 'expanding' uses progressively larger evaluation windows with recency weighting.", "type": "select", "options": ["expanding", "latest_window", "fixed_window"]},
            "competition.strategy_params.window_months": {"label": "Evaluation Window", "description": "Number of recent months used to evaluate model accuracy.", "type": "integer", "min": 3, "max": 24, "unit": "months"},
            "competition.strategy_params.decay_factor": {"label": "Recency Decay Factor", "description": "Weight decay for older months in accuracy evaluation. 0.90 means each older month gets 90% of the previous month's weight.", "type": "number", "min": 0.5, "max": 1.0, "step": 0.01},
            "competition.strategy_params.top_k": {"label": "Top K Candidates", "description": "Number of best-performing models to consider as champion candidates.", "type": "integer", "min": 1, "max": 10},
            "competition.strategy_params.weight_method": {"label": "Weight Method", "description": "How accuracy is scored: 'inverse_wape' gives higher scores to lower WAPE values.", "type": "select", "options": ["inverse_wape", "accuracy_pct", "mape"]},
            "competition.meta_learner.model_type": {"label": "Meta-Learner Algorithm", "description": "ML algorithm used by the meta-learner to predict which base model performs best for each DFU.", "type": "select", "options": ["random_forest", "logistic_regression", "gradient_boosting"]},
            "competition.meta_learner.n_estimators": {"label": "Meta-Learner Trees", "description": "Number of trees in the meta-learner ensemble.", "type": "integer", "min": 50, "max": 1000, "step": 50},
            "competition.meta_learner.max_depth": {"label": "Meta-Learner Depth", "description": "Maximum depth of meta-learner trees.", "type": "integer", "min": 3, "max": 20},
            "competition.meta_learner.test_months": {"label": "Meta-Learner Test Window", "description": "Months of holdout data for meta-learner evaluation.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
        },
    },
    "production_forecast_config": {
        "label": "Production Forecast",
        "category": "forecasting",
        "description": "Production forecast generation pipeline settings. Controls inference horizon, confidence intervals, model selection strategy, and output versioning.",
        "fields": {
            "inference.horizon_months": {"label": "Forecast Horizon", "description": "Number of months ahead to generate forecasts.", "type": "integer", "min": 1, "max": 36, "unit": "months"},
            "inference.recursive": {"label": "Recursive Inference", "description": "Use recursive multi-step forecasting instead of direct.", "type": "boolean"},
            "confidence_interval.enabled": {"label": "Confidence Intervals", "description": "Generate prediction intervals (P10/P90) around the point forecast.", "type": "boolean"},
            "confidence_interval.z_lower": {"label": "CI Z-Score Lower", "description": "Z-score for lower confidence bound. 1.282 gives P10.", "type": "number", "min": 0.5, "max": 3.0, "step": 0.001},
            "confidence_interval.z_upper": {"label": "CI Z-Score Upper", "description": "Z-score for upper confidence bound. 1.282 gives P90.", "type": "number", "min": 0.5, "max": 3.0, "step": 0.001},
            "confidence_interval.sigma_floor": {"label": "Sigma Floor", "description": "Minimum prediction standard deviation. Prevents unrealistically narrow confidence intervals.", "type": "number", "min": 0.0, "max": 10.0, "step": 0.1},
            "confidence_interval.sigma_cap_multiplier": {"label": "Sigma Cap Multiplier", "description": "Cap sigma at this multiple of the median forecast.", "type": "number", "min": 1.0, "max": 10.0, "step": 0.5},
            "confidence_interval.horizon_scaling": {"label": "Horizon Scaling Method", "description": "How uncertainty grows with forecast horizon: 'sqrt' scales sigma by sqrt(h), 'linear' scales by h.", "type": "select", "options": ["sqrt", "linear", "none"]},
            "model_selection.strategy": {"label": "Model Selection Strategy", "description": "'champion' uses the pre-selected best model per DFU; 'latest' uses the most recently trained model.", "type": "select", "options": ["champion", "latest", "specific"]},
            "model_selection.fallback_model_id": {"label": "Fallback Model", "description": "Model to use when champion assignment is unavailable for a DFU.", "type": "text"},
            "plan_version.keep_last_n_versions": {"label": "Max Plan Versions", "description": "Maximum number of forecast plan versions to retain.", "type": "integer", "min": 1, "max": 20},
    ...
    "quantile_forecast_config": {
        ...
        "category": "forecasting",
        "description": "Quantile regression forecast settings for generating probabilistic demand estimates at specific percentiles (P10, P50, P90).",
        "fields": {
    ...
    "seasonality_config": {
        ...
        "category": "forecasting",
        "description": "Seasonal pattern detection and profiling settings. Used to classify DFUs by seasonality strength and assign profiles for model features.",
        "fields": {
            "seasonality.min_months_history": {"label": "Min History", "description": "Minimum months of sales history required before seasonality analysis is attempted.", "type": "integer", "min": 6, "max": 48, "unit": "months"},
    ...
    "fva_config": {
        "label": "Forecast Value Added (FVA)",
        "category": "forecasting",
        "description": "FVA measurement configuration — tracks whether human overrides and system adjustments actually improve forecast accuracy.",
        "fields": {
            "measurement_windows.forecast_override": {"label": "Override Window", "description": "Months to wait after a forecast override before measuring its accuracy impact.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
            "measurement_windows.ss_change": {"label": "Safety Stock Window", "description": "Months to evaluate the impact of safety stock parameter changes.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
            "measurement_windows.policy_change": {"label": "Policy Change Window", "description": "Months to measure the effect of replenishment policy changes.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
            "measurement_windows.ai_insight_action": {"label": "AI Insight Window", "description": "Months to evaluate whether acting on AI planner insights improved outcomes.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
            "measurement_windows.sop_approval": {"label": "S&OP Approval Window", "description": "Months to measure the impact of S&OP consensus adjustments.", "type": "integer", "min": 1, "max": 12, "unit": "months"},
            "financial_impact.carrying_cost_pct": {"label": "Carrying Cost %", "description": "Annual carrying cost as a fraction of inventory value.", "type": "number", "min": 0.01, "max": 1.0, "step": 0.01, "unit": "%"},
            "financial_impact.stockout_cost_per_unit": {"label": "Stockout Cost/Unit", "description": "Estimated cost per unit of unmet demand.", "type": "number", "min": 0, "max": 1000, "unit": "$"},
            "financial_impact.service_level_target": {"label": "Target Service Level", "description": "Service level target used for FVA financial impact calculations.", "type": "number", "min": 0.8, "max": 0.999, "step": 0.005},
        },
    },
    "bias_correction_config": {
        "label": "Bias Correction",
        "category": "forecasting",
        "description": "Automatic forecast bias detection and correction. Identifies DFUs with systematic over- or under-forecasting and applies weighted rolling corrections.",
        "fields": {
    ...
    "variability_config": {
        ...
        "category": "inventory",
        "description": "Demand variability analysis and classification. Computes CV and intermittency for each DFU to drive policy assignment.",
        "fields": {
            "history.history_months": {"label": "History Months", "description": "Number of months of sales history to analyze for variability.", "type": "integer", "min": 6, "max": 60, "unit": "months"},
            "history.min_months_history": {"label": "Min History Months", "description": "Minimum months required before computing variability.", "type": "integer", "min": 3, "max": 24, "unit": "months"},
            "outlier.sigma_threshold": {"label": "Outlier Sigma", "description": "Winsorization threshold in standard deviations. Values beyond this are capped before CV computation.", "type": "number", "min": 1.0, "max": 5.0, "step": 0.5},
            "cv_thresholds.low": {"label": "CV — Low Threshold", "description": "CV below this is classified as 'stable' (X-class).", "type": "number", "min": 0.05, "max": 0.5, "step": 0.05},
            "cv_thresholds.medium": {"label": "CV — Medium Threshold", "description": "CV between low and this is 'moderate' (Y-class).", "type": "number", "min": 0.3, "max": 1.5, "step": 0.05},
            "cv_thresholds.high": {"label": "CV — High Threshold", "description": "CV above this is 'volatile' (Z-class).", "type": "number", "min": 0.5, "max": 3.0, "step": 0.1},
            "intermittency_threshold.ratio": {"label": "Intermittency Threshold", "description": "Fraction of months with zero demand above which the item is classified as intermittent.", "type": "number", "min": 0.1, "max": 0.8, "step": 0.05},
        },
    },
    "lead_time_config": {
        "label": "Lead Time Analysis",
        "category": "inventory",
        "description": "Lead time variability analysis and change point detection.",
        "fields": {
            "history.history_months": {"label": "History Window", "description": "Months of receipt history to analyze for lead time patterns.", "type": "integer", "min": 3, "max": 36, "unit": "months"},
            "change_point.min_observations": {"label": "Min Change Points", "description": "Minimum number of data points required for change-point detection algorithm.", "type": "integer", "min": 2, "max": 10},
            "cv_thresholds.stable": {"label": "Stable LT Threshold", "description": "LT CV below this is 'stable'.", "type": "number", "min": 0.0, "max": 0.3, "step": 0.01},
            "cv_thresholds.moderate": {"label": "Moderate LT Threshold", "description": "LT CV between stable and this is 'moderate'.", "type": "number", "min": 0.1, "max": 0.6, "step": 0.01},
            "batch.batch_size": {"label": "Batch Size", "description": "Rows per database batch upsert.", "type": "integer", "min": 100, "max": 5000},
        },
    },
    "simulation_config": {
        "label": "Monte Carlo Simulation",
        "category": "inventory",
        "description": "Monte Carlo simulation settings for safety stock optimization.",
        "fields": {
            ...
            "simulation.max_dfus_per_batch": {"label": "Max DFUs per Batch", "description": "DFUs processed in each batch to manage memory.", "type": "integer", "min": 50, "max": 2000},
        },
    },
    ...
    "projection_config": {
        ...
        "category": "operations",
        "description": "Forward inventory projection settings.",
        "fields": {
            "projection.horizon_days": {"label": "Projection Horizon", "description": "Days ahead to project inventory levels.", "type": "integer", "min": 14, "max": 365, "unit": "days"},
            "projection.scenarios": {"label": "Projection Scenarios", "description": "List of scenarios to model: 'no_order', 'with_open_po', 'with_planned_orders'.", "type": "array"},
            "thresholds.reorder_point_source": {"label": "Reorder Point Source", "description": "Source for reorder points: 'safety_stock' uses computed SS, 'manual' uses planner-set ROP.", "type": "select", "options": ["safety_stock", "manual"]},
            "thresholds.excess_coverage_months": {"label": "Excess Coverage", "description": "Months of forward demand coverage above which inventory is excess.", "type": "integer", "min": 1, "max": 18, "unit": "months"},
        },
    },
    ...
    "demand_signals_external_config": {
        "label": "External Demand Signals",
        "category": "pipeline",
        "description": "External data source integration for demand signal enrichment.",
        "fields": {
            "sources.weather.provider": {"label": "Weather Provider", "description": "Weather data API provider.", "type": "text"},
            "sources.economic.provider": {"label": "Economic Provider", "description": "Economic indicator data source.", "type": "text"},
            "decomposition.period": {"label": "STL Period", "description": "Period for STL decomposition. 12 for monthly data = annual seasonality.", "type": "integer", "min": 2, "max": 52},
            "decomposition.seasonal_window": {"label": "STL Seasonal Window", "description": "Window size for seasonal component smoother. Must be odd.", "type": "integer", "min": 3, "max": 51},
        },
    },
    "exception_config": {
    ...
    "api_governance_config": {
        "label": "API Governance",
        "category": "system",
        "description": "API rate limiting tiers and versioning policy.",
        "fields": {
            "rate_limit_tiers.free.requests_per_minute": {"label": "Free Tier RPM", "description": "Maximum API requests per minute for free-tier users.", "type": "integer", "min": 10, "max": 1000},
            "rate_limit_tiers.standard.requests_per_minute": {"label": "Standard Tier RPM", "description": "Maximum requests per minute for standard-tier users.", "type": "integer", "min": 50, "max": 5000},
            "rate_limit_tiers.premium.requests_per_minute": {"label": "Premium Tier RPM", "description": "Maximum requests per minute for premium-tier users.", "type": "integer", "min": 100, "max": 10000},
            "deprecation.sunset_header": {"label": "Sunset Header", "description": "Include HTTP Sunset header for deprecated API versions.", "type": "boolean"},
            "versioning.current_version": {"label": "Current API Version", "description": "Active API version identifier.", "type": "text"},
        },
    },
    "notification_config": {
        "label": "Notifications",
    ...
    "reporting_config": {
        ...
        "fields": {
            "pdf.branding": {"label": "Company Name", "description": "Company name displayed on PDF report headers.", "type": "text"},
            "delivery_retry.max_attempts": {"label": "Delivery Retries", "description": "Number of delivery attempts for failed report emails.", "type": "integer", "min": 0, "max": 10},
            "delivery_retry.backoff_seconds": {"label": "Retry Backoff", "description": "Seconds to wait between delivery retries.", "type": "integer", "min": 10, "max": 600, "unit": "sec"},
        },
    },
    "ai_planner_config": {
        "label": "AI Planner",
