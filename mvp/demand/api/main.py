...
def _google_search(query: str, num_results: int = 5) -> list[dict[str, str]]:
    """Call Google Custom Search JSON API. Returns list of {title, link, snippet}."""
    api_key = os.getenv("GOOGLE_API_KEY", "")
    cse_id = os.getenv("GOOGLE_CSE_ID", "")
    if not api_key or not cse_id:
        raise HTTPException(status_code=503, detail="Google Search API not configured (set GOOGLE_API_KEY and GOOGLE_CSE_ID)")
...
def _lookup_item_context(item_no: str) -> dict[str, str | None]:
    """Fetch item metadata for search query construction."""
    sql = """
        SELECT item_desc, brand_name, category, producer_name, supplier_name
        FROM dim_item
        WHERE item_no = %s
        LIMIT 1
    """
    with get_conn() as conn, conn.cursor() as cur:
        ...
        row = cur.fetchone()
    if not row:
        return {"item_desc": None, "brand_name": None, "category": None,
                "producer_name": None, "supplier_name": None}
    return {
        "item_desc": row[0],
        "brand_name": row[1],
        "category": row[2],
...
def _lookup_location_context(location_id: str) -> dict[str, str | None]:
    """Fetch location metadata for demographic context."""
    sql = """
        SELECT state_id, site_desc
        FROM dim_location
        WHERE location_id = %s
        LIMIT 1
    """
    with get_conn() as conn, conn.cursor() as cur:
        ...
        row = cur.fetchone()
    if not row:
        return {"state_id": None, "site_desc": None}
    return {"state_id": row[0], "site_desc": row[1]}


def _build_search_query(item_ctx: dict[str, str | None]) -> str:
    """Build Google search query from item metadata."""
    parts: list[str] = []
    if item_ctx.get("item_desc"):
        parts.append(item_ctx["item_desc"])  # type: ignore[arg-type]
    if item_ctx.get("brand_name"):
        parts.append(item_ctx["brand_name"])  # type: ignore[arg-type]
    if item_ctx.get("category"):
        parts.append(item_ctx["category"])  # type: ignore[arg-type]
    query_base = " ".join(parts) if parts else "consumer product"
    return f"{query_base} market trends news demand"


MARKET_INTEL_SYSTEM_PROMPT = """You are a market intelligence analyst for a demand forecasting team.
Given product information, web search results about the product/market, and a target location (US state),
synthesize a concise market intelligence narrative.
...
## Instructions
1. Summarize key market trends, news, and competitive dynamics for this product category.
2. Provide demographic and economic context for the target state that could affect demand.
   (Use your knowledge of US state demographics — population, income levels, urbanization,
   consumer preferences, regulatory environment for this product category.)
3. Identify potential demand drivers or risks based on the combined product + location context.
4. Keep the narrative concise but insightful (3-5 paragraphs).
5. Structure your response with clear section headers using markdown (## headers).
6. Focus on actionable insights for a demand planner.

Respond with a well-structured markdown narrative. Do NOT use JSON format."""


@app.post("/market-intelligence")
def market_intelligence(req: MarketIntelRequest):
    """Generate market intelligence narrative for an item + location pair."""
    item_no = req.item_no.strip()
    location_id = req.location_id.strip()
    if not item_no:
        raise HTTPException(422, "item_no is required")
    if not location_id:
        raise HTTPException(422, "location_id is required")

    # 1. Lookup item + location context from Postgres
    item_ctx = _lookup_item_context(item_no)
    loc_ctx = _lookup_location_context(location_id)

    if not item_ctx.get("item_desc"):
        raise HTTPException(404, f"Item '{item_no}' not found in dim_item")
    if not loc_ctx.get("state_id"):
        raise HTTPException(404, f"Location '{location_id}' not found in dim_location")

    # 2. Google Custom Search
    search_query = _build_search_query(item_ctx)
    search_results: list[dict[str, str]] = []
    try:
        search_results = _google_search(search_query, num_results=5)
    except HTTPException:
        raise
    except Exception:
        pass  # Graceful degradation — continue without search results
    ...
    for k, v in item_ctx.items():
        if v:
            ...
            product_lines.append(f"- {label}: {v}")
    product_context = "\n".join(product_lines) if product_lines else "(no product details available)"

    search_lines: list[str] = []
    for i, sr in enumerate(search_results, 1):
        search_lines.append(f"{i}. **{sr['title']}**\n   {sr['snippet']}\n   Source: {sr['link']}")
    search_context = "\n".join(search_lines) if search_lines else "(no search results available)"

    system_prompt = MARKET_INTEL_SYSTEM_PROMPT.format(
        product_context=product_context,
        state_id=loc_ctx.get("state_id", "Unknown"),
        site_desc=loc_ctx.get("site_desc", "Unknown"),
    ...
    chat_resp = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Generate a market intelligence briefing for {item_ctx.get('item_desc', item_no)} in {loc_ctx.get('state_id', 'the target location')}."},
        ],
        temperature=0.4,
        max_tokens=2000,
    )
    narrative = chat_resp.choices[0].message.content or "Unable to generate narrative."

    # 5. Return combined response
    return {
        "item_no": item_no,
        "location_id": location_id,
        "item_desc": item_ctx.get("item_desc"),
        "brand_name": item_ctx.get("brand_name"),
        "category": item_ctx.get("category"),
        "state_id": loc_ctx.get("state_id"),
        "site_desc": loc_ctx.get("site_desc"),
        "search_results": [
