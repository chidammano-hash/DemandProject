...
def train_and_predict_transfer(
    ...
    Phase 1: Train a base LGBM on ALL training data, excluding ml_cluster from features.
    Phase 2: For each cluster with >= transfer_min_rows: fine-tune from the base model
             with transfer_n_estimators additional trees.
    ...
    for ci, cluster_label in enumerate(clusters, 1):
        ...
        # Fine-tune: new LGBM initialized from base model's booster
...
def main() -> None:
    ...
    for ti, tf in enumerate(timeframes):
        ...
        if args.cluster_strategy == "global":
            preds, model = train_and_predict_global(
                train_data, predict_data, feature_cols, cat_cols, lgbm_params
            )
    ...
    # Compute accuracy summary — all rows are already at execution lag
    ...
    try:
        ...
        with mlflow.start_run():
            mlflow.set_tag("model_type", "lgbm_backtest")
