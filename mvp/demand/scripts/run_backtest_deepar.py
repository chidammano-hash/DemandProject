"""
Run DeepAR backtesting with expanding-window timeframes.

DeepAR (Deep AutoRegressive) uses a stacked LSTM encoder conditioned on static
covariates, with a Gaussian distribution head that outputs (mu, sigma) for
probabilistic forecasting.  The point forecast is mu.  Unlike PatchTST which
uses a Transformer, DeepAR processes the sequence autoregressively through
LSTM layers, naturally capturing temporal dependencies.

Supports three strategies:
  - global:      One DeepAR trained on all DFUs           (model_id=deepar_global)
  - per_cluster: Separate DeepAR per ml_cluster           (model_id=deepar_cluster)
  - transfer:    Global base -> per-cluster fine-tune      (model_id=deepar_transfer)
...
def _train_and_predict_global(
    ...
    """Train one global DeepAR and predict all DFUs."""
    import torch
    from torch.utils.data import DataLoader, random_split

    from scripts.deepar_model import (
        DeepARModel,
        DemandDataset,
        predict_model,
        train_model,
    ...
    mu_log1p, sigma_log1p = predict_model(model, full_dataset, device, batch_size=train_params["batch_size"])

    # Convert from log1p space -> raw qty, floor at 0 (point forecast = mu)
    preds_raw = np.expm1(mu_log1p)
...
def _train_and_predict_per_cluster(
    ...
    """Train separate DeepAR per ml_cluster."""
    import torch
    from torch.utils.data import DataLoader, random_split
    ...
    for ci, cluster_label in enumerate(clusters, 1):
        ...
        mu_log1p, sigma_log1p = predict_model(model, full_ds, device, batch_size=train_params["batch_size"])
        preds_raw = np.maximum(np.expm1(mu_log1p), 0)
...
def _train_and_predict_transfer(
    ...
    """Transfer learning: global base model -> per-cluster fine-tune with frozen LSTM layers."""
    import torch
    from torch.utils.data import DataLoader, random_split
    ...
    print(f"      Phase 1: Training global base DeepAR ({n_total:,} sequences)...")
    t0 = time.time()
    ...
    base_model = DeepARModel(**arch_params)
    history = train_model(
    ...
    for ci, cluster_label in enumerate(clusters, 1):
        ...
        if len(idxs) < transfer_min_rows:
            ...
            mu_log1p, _ = predict_model(base_model, pred_ds, device, batch_size=train_params["batch_size"])
            preds_raw = np.maximum(np.expm1(mu_log1p), 0)
            for j, idx in enumerate(idxs):
        ...
        t1 = time.time()
        ft_model = DeepARModel(**arch_params)
        ...
        pred_ds = DemandDataset(c_seqs, c_covs, targets=None)
        mu_log1p, _ = predict_model(ft_model, pred_ds, device, batch_size=train_params["batch_size"])
        preds_raw = np.maximum(np.expm1(mu_log1p), 0)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run DeepAR backtest with expanding-window timeframes")
    ...
    parser.add_argument("--model-id", type=str, default=None,
                        help="Override model_id (default: deepar_global, deepar_cluster, or deepar_transfer)")
    ...
    # DeepAR architecture args
    parser.add_argument("--seq-len", type=int, default=12, help="Input sequence length (months)")
    parser.add_argument("--hidden-size", type=int, default=64, help="LSTM hidden dimension")
    ...
    parser.add_argument("--transfer-freeze-layers", type=int, default=1,
                        help="Number of LSTM layers to freeze during fine-tuning")
    ...
    from scripts.deepar_model import get_device

    device = get_device(args.device)

    _default_model_ids = {
        "global": "deepar_global",
        "per_cluster": "deepar_cluster",
    ...
    print(f"[{_ts()}] DeepAR Backtest: strategy={args.cluster_strategy}, model_id={model_id}, "
    ...
    deepar_params = {**arch_params, **train_params}
    if args.cluster_strategy == "transfer":
        deepar_params["transfer_epochs"] = args.transfer_epochs
        deepar_params["transfer_min_rows"] = args.transfer_min_rows
        deepar_params["transfer_freeze_layers"] = args.transfer_freeze_layers
    ...
    output_path, archive_path, meta_path, metadata = save_backtest_output(
        ...
        model_params=deepar_params,
        model_params_key="deepar_params",
    ...
    print(f"\n[{_ts()}] DeepAR backtest complete in {elapsed:.0f}s ({elapsed / 60:.1f}m)")
