...
import type { VariabilityDetailRow, LtProfileRow } from "@/api/queries";
import type {
  InventoryPosition,
  InventoryKpis,
  InventoryTrendPoint,
...
export function InventoryTab() {
  ...
  const [ltClassFilter, setLtClassFilter] = useState("");
  ...
  const ltProfileParams = useMemo(
    () => ({
      lt_variability_class: ltClassFilter || undefined,
      limit: LT_PAGE,
      offset: ltOffset,
      sort_by: "lt_cv",
      sort_dir: "desc",
    }),
    [ltClassFilter, ltOffset],
  );
  ...
  return (
    <section className="mt-4 space-y-4">
      ...
      {/* ------------------------------------------------------------------ */}
      {/* IPfeature2: Lead Time Profile Panel                                 */}
      {/* ------------------------------------------------------------------ */}
      <Card>
        <CardHeader
          className="cursor-pointer select-none"
          onClick={() => setLtPanelOpen((o) => !o)}
        >
          <div className="flex items-center justify-between">
            <div>
        ...
        {ltPanelOpen && (
          <CardContent className="space-y-4">
            {loadingLtSummary ? (
              <LoadingElement tabKey="lt-summary" message="Loading LT summary..." />
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(["stable", "moderate", "volatile"] as const).map((cls) => {
                    ...
                    return (
                      <button
                        key={cls}
                        onClick={() => {
                          setLtClassFilter(ltClassFilter === cls ? "" : cls);
                          setLtOffset(0);
                        }}
                        className={cn(
                          "flex flex-col items-center rounded-lg border px-4 py-2 text-sm transition-colors",
                          colorMap[cls],
                          ltClassFilter === cls && "ring-2 ring-primary",
                        )}
                      >
                        <span className="text-xl font-bold tabular-nums">
                  ...
                  <div className="flex flex-col items-center rounded-lg border border-border px-4 py-2 text-sm">
                    <span className="text-xl font-bold tabular-nums">
                      {ltSummary.avg_lt_cv != null ? Number(ltSummary.avg_lt_cv).toFixed(2) : "—"}
                    </span>
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {ltSummary.lt_cv_p50 != null ? Number(ltSummary.lt_cv_p50).toFixed(2) : "—"} · p95:{" "}
                  {ltSummary.lt_cv_p95 != null ? Number(ltSummary.lt_cv_p95).toFixed(2) : "—"}
                </div>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">
                No lead time profile data. Run <code>make lt-profile-compute</code> first.
              </p>
            )}

            {/* Detail table */}
            {loadingLtProfile ? (
              <LoadingElement tabKey="lt-profile" message="Loading profiles..." />
            ) : ltRows.length > 0 ? (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">
                    {ltClassFilter
                      ? `${ltClassFilter.charAt(0).toUpperCase() + ltClassFilter.slice(1)} LT DFUs`
                    ...
                    <TableBody>
                      {ltRows.map((r) => {
                        ...
                        return (
                          <TableRow
                            key={`${r.item_no}-${r.loc}`}
                            className={tierColor[r.lt_variability_class ?? ""] ?? ""}
                          >
                            <TableCell className="font-mono text-xs">{r.item_no}</TableCell>
                            <TableCell className="text-xs">{r.loc}</TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                              {r.lt_mean_days != null ? `${Number(r.lt_mean_days).toFixed(1)}d` : "—"}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                              {r.lt_std_days != null ? `${Number(r.lt_std_days).toFixed(1)}d` : "—"}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                              {r.lt_cv != null ? Number(r.lt_cv).toFixed(3) : "—"}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                              {r.lt_p95_days != null ? `${Number(r.lt_p95_days).toFixed(1)}d` : "—"}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                              {r.observation_count ?? "—"}
                            </TableCell>
                            <TableCell>
                              <span
                                className={cn(
                                  "rounded px-1.5 py-0.5 text-xs font-medium capitalize",
                                  badgeColor[r.lt_variability_class ?? ""] ?? "",
                                )}
                              >
                ...
                {(ltProfileData?.total ?? 0) > LT_PAGE && (
                  <div className="flex items-center justify-between text-sm">
                    <button
                      disabled={ltOffset === 0}
                      onClick={() => setLtOffset(Math.max(0, ltOffset - LT_PAGE))}
                      className="flex items-center gap-1 disabled:opacity-40"
                    >
                      <ChevronLeft className="h-4 w-4" /> Prev
                    </button>
                    <span className="text-muted-foreground">
                      {ltOffset + 1}–{Math.min(ltOffset + LT_PAGE, ltProfileData?.total ?? 0)} of{" "}
                      {ltProfileData?.total}
                    </span>
                    <button
                      disabled={ltOffset + LT_PAGE >= (ltProfileData?.total ?? 0)}
            ...
            ) : ltPanelOpen && !loadingLtProfile ? (
              <p className="text-sm text-muted-foreground">No lead time profiles available.</p>
            ) : null}
          </CardContent>
        )}
      </Card>
