...
// ---------------------------------------------------------------------------
// Inline fetch functions (no dependency on queries.ts to avoid conflicts)
// ---------------------------------------------------------------------------
async function fetchSbSummary(): Promise<StoryboardSummary> {
  const res = await fetch("/storyboard/exceptions/summary");
  if (!res.ok) throw new Error("Failed to fetch storyboard summary");
  return res.json();
}

async function fetchSbExceptions(params: {
  status?: string;
...
async function fetchSbException(
  id: string
...
async function updateSbStatus(id: string, status: string): Promise<void> {
...
async function submitSbDecision(
  id: string,
  decisionType: string,
  rationale: string
): Promise<void> {
