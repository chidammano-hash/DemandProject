...
def _process_timeframe(
    ...
    wlog = logging.getLogger(f"chronos_bolt.worker.{ti}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Chronos Bolt (v2) foundation model backtest (zero-shot, all DFUs)",
    )
    parser.add_argument(
    ...
    with profiled_section("load_config"):
        ...
        bolt_cfg = cfg.get("algorithms", {}).get(CONFIG_KEY, {})
        if not bolt_cfg.get("enabled", True):
            logger.info("Chronos Bolt is disabled in config; exiting")
            return
        ...
        model_id = bolt_cfg.get("model_id", MODEL_ID)

        bolt_params = {
            "model_size": bolt_cfg.get("model_size", "base"),
            "device": bolt_cfg.get("device", "auto"),
            "batch_size": bolt_cfg.get("batch_size", 1024),
            "num_samples": bolt_cfg.get("num_samples", 12),
            "prediction_length": bolt_cfg.get("prediction_length", 6),
        }
    ...
    if args.workers > 1:
        ...
        n_workers_cfg = bolt_cfg.get("num_workers", 1)
        if n_workers_cfg > 1:
            args.workers = n_workers_cfg

    logger.info(
        "Chronos Bolt backtest config: model_id=%s, n_timeframes=%d, "
        "embargo_months=%d, model_size=%s, batch_size=%d, workers=%d",
        model_id, n_timeframes, embargo_months,
        bolt_params["model_size"], bolt_params["batch_size"],
        args.workers,
    )
    ...
    # ── Step 3: Run Chronos Bolt per timeframe ─────────────────────────────
    n_workers = args.workers
    logger.info(
        "Step 3: Running Chronos Bolt inference across %d timeframes (%d worker%s)...",
    ...
    if n_workers > 1:
        # ── Parallel path ──────────────────────────────────────────────
        tmp_dir = tempfile.mkdtemp(prefix="bolt_bt_")
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as pool:
            futures = {
                pool.submit(
                    ...
                    sales_path, all_months_ser, bolt_params,
        ...
        for ti, tf in enumerate(timeframes):
            ...
            with profiled_section(f"bolt_tf_{label}"):
                preds = run_foundation_models(
                    train_sales[["sku_ck", "startdate", "qty"]],
                    predict_months,
                    {DISPATCHER_KEY: bolt_params},
    ...
    logger.info(
        "Chronos Bolt backtest complete: accuracy=%.2f%%, %s predictions, %.1f min total",
