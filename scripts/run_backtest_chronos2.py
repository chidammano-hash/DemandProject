...
def _process_timeframe(
    ...
    wlog = logging.getLogger(f"chronos2.worker.{ti}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Chronos 2 foundation model backtest (zero-shot, all DFUs)",
    )
    parser.add_argument(
    ...
    with profiled_section("load_config"):
        ...
        c2_cfg = cfg.get("algorithms", {}).get(CONFIG_KEY, {})
        if not c2_cfg.get("enabled", True):
            logger.info("Chronos 2 is disabled in config; exiting")
            return
        ...
        model_id = c2_cfg.get("model_id", MODEL_ID)

        c2_params = {
            "device": c2_cfg.get("device", "auto"),
            "batch_size": c2_cfg.get("batch_size", 1024),
            "prediction_length": c2_cfg.get("prediction_length", 6),
        }
    ...
    if args.workers > 1:
        ...
        n_workers_cfg = c2_cfg.get("num_workers", 1)
        if n_workers_cfg > 1:
            args.workers = n_workers_cfg

    logger.info(
        "Chronos 2 backtest config: model_id=%s, n_timeframes=%d, "
        "embargo_months=%d, batch_size=%d, workers=%d",
        model_id, n_timeframes, embargo_months,
        c2_params["batch_size"], args.workers,
    )
    ...
    # ── Step 3: Run Chronos 2 per timeframe ─────────────────────────────
    n_workers = args.workers
    logger.info(
        "Step 3: Running Chronos 2 inference across %d timeframes (%d worker%s)...",
    ...
    if n_workers > 1:
        # ── Parallel path ──────────────────────────────────────────────
        tmp_dir = tempfile.mkdtemp(prefix="chronos2_bt_")
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as pool:
            futures = {
                pool.submit(
                    ...
                    sales_path, all_months_ser, c2_params,
        ...
        for ti, tf in enumerate(timeframes):
            ...
            with profiled_section(f"c2_tf_{label}"):
                preds = run_foundation_models(
                    train_sales[["sku_ck", "startdate", "qty"]],
                    predict_months,
                    {DISPATCHER_KEY: c2_params},
    ...
    logger.info(
        "Chronos 2 backtest complete: accuracy=%.2f%%, %s predictions, %.1f min total",
