...
def fetch_network_topology(
    ...
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT item_no, dc_loc, store_loc, echelon_level,
                   replenishment_lt_days, lt_std_dev_days
            FROM dim_echelon_network
            WHERE {where}
            ORDER BY item_no, dc_loc, store_loc
            """,
            params,
        )
        rows = cur.fetchall()
    return [
        {
            ...
            "store_loc": r[2],
            "echelon_level": r[3],
            "lt_days": float(r[4]) if r[4] else 10.0,
            "lt_std_days": float(r[5]) if r[5] else 2.0,
        }
        for r in rows
    ]
...
def fetch_downstream_stats(
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM dim_dfu d
            WHERE d.dmdunit = %s AND d.loc = ANY(%s)
            """,
            (item_no, store_locs),
        )
        rows = cur.fetchall()
...
def upsert_echelon_ss_targets(conn: psycopg.Connection, rows: list[dict]) -> int:
    sql = """
        INSERT INTO fact_echelon_ss_targets (
            item_no, dc_loc, plan_date, downstream_node_count,
            ...
            %(item_no)s, %(dc_loc)s, %(plan_date)s, %(downstream_node_count)s,
        ...
        ON CONFLICT (item_no, dc_loc, plan_date)
        DO UPDATE SET
...
def run(
    ...
    with psycopg.connect(**get_db_params()) as conn:
        topology = fetch_network_topology(conn, item_no)

        # Group by (item_no, dc_loc)
        from collections import defaultdict
        dc_groups: dict = defaultdict(list)
        dc_lt: dict = {}
        for node in topology:
            key = (node["item_no"], node["dc_loc"])
        ...
        for (item, dc), nodes in dc_groups.items():
            ...
            downstream = fetch_downstream_stats(conn, item, store_locs)
            ...
            dc_on_hand = fetch_dc_on_hand(conn, item, dc)
