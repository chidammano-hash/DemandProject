...
function SkuForecastChart({
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchProductionForecast({ item_id: item, loc, plan_version: planVersion }),
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
        <span>
          ...
          {data.promoted_run && (
            <span className="ml-1 inline-flex items-center rounded-full bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300 px-2 py-0.5 text-[10px] font-semibold">
              ...
              {data.promoted_run.accuracy_pct != null &&
                ` · ${data.promoted_run.accuracy_pct.toFixed(1)}%`}
            </span>
          )}
        </span>
        <span>
        ...
        {data?.generated_at &&
          new Date(data.generated_at) < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) && (
            <p className="text-xs text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded mt-1">
              ⚠ Forecast is more than 7 days old. Run{" "}
              <code className="font-mono text-[11px]">make forecast-generate</code> to refresh.
            </p>
          )}
        {data.is_recursive && (
      ...
      <p className="text-xs text-muted-foreground mt-1">
        <strong>P10</strong> (dashed lower) = conservative floor · <strong>P50</strong> (solid) =
        median forecast · <strong>P90</strong> (dashed upper) = cautious ceiling. Use P10 for safety
        stock, P50 for consensus demand planning.
      </p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      {!versionsLoading && (versions?.versions?.length ?? 0) === 0 && (
        <EmptyState
          ...
          description="Production forecasts use the retained LightGBM, N-HiTS, N-BEATS, MSTL, and Chronos 2E roster. Each DFU gets a monthly forecast with P10/P90 confidence bands derived from backtest residuals."
          steps={[
      ...
      {summaryLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          />
          <KpiCard label="Horizon" value={`${horizonMonths} months`} sub="forward-looking window" />
        </div>
      ) : null}
      ...
      {abcChartData.length > 0 && (
        <div className="rounded-lg border bg-card p-4 space-y-3">
          ...
          <p
            ...
            title="A = top 80% of demand value · B = next 15% · C = bottom 5%"
          >
            Forecast by ABC Class{" "}
            <span className="cursor-help underline decoration-dotted">(?)</span>
          </p>
      ...
      <div className="rounded-lg border bg-card p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Enter exact item and location codes from master data.
        </p>
