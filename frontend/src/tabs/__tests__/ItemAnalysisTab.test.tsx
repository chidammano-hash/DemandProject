...
vi.mock("@/api/queries/core", () => ({
  ...
  fetchShapSummary: vi
    .fn()
    .mockResolvedValue({ model_id: "lgbm_cluster", total_features: 0, features: [] }),
}));
...
vi.mock("@/api/queries/production-forecast", () => ({
  ...
  fetchStagingForecasts: vi
    .fn()
    .mockResolvedValue({ item_id: "100320", loc: "1401-BULK", models: {} }),
  fetchCandidateForecasts: vi
    .fn()
    .mockResolvedValue({ item_id: "100320", loc: "1401-BULK", models: {} }),
}));
...
describe("ItemAnalysisTab", () => {
  beforeEach(() => {
    try {
      localStorage.removeItem("ds:itemAnalysis:panels");
    } catch {
      /* no-op */
    }
  });
  ...
  it("explains why SHAP and AI Champion panels are empty instead of rendering nothing", async () => {
    ...
    try {
      renderTab();
      expect(await screen.findByText("No SHAP data is available for this DFU.")).toBeDefined();
      expect(
