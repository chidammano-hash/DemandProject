      ...
      <div ref={scenarioResultRef} className="space-y-3 rounded-lg border border-border p-4">
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold">
            ...
            {(
              scenarioResult.result.total_dfus ?? scenarioResult.result.total_skus
            )?.toLocaleString()}{" "}
            SKUs, {scenarioResult.runtime_seconds.toFixed(1)}s
          </p>
          <button
            ...
            <TableBody>
              {scenarioResult.result.profiles.map((p) => (
                <TableRow key={p.label}>
                  <TableCell className="text-sm font-medium" title={p.label}>
                    {formatClusterLabel(p.label)}
                  </TableCell>
        ...
        <ScenarioCharts
          ...
          pcaScatter={scenarioResult.result.pca_scatter}
        />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-lg border border-border bg-card p-6 shadow-xl">
            <p className="text-base font-semibold">
              Promote Scenario {scenarioLabel} to Production?
            </p>
            ...
            <div className="mt-3 rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/30 p-3">
              ...
              <ul className="mt-1.5 space-y-1 text-xs text-amber-700 dark:text-amber-400">
                <li>
                  • <strong>Cluster assignments</strong> in{" "}
                  <code className="text-[10px] bg-amber-100 dark:bg-amber-900/50 rounded px-0.5">
                    dim_sku.ml_cluster
                  </code>{" "}
                  will be overwritten for all SKUs
                </li>
                <li>
                  • <strong>Backtesting</strong> — LightGBM will train on the new cluster
                  boundaries. Previous backtest results may no longer be comparable.
                </li>
                <li>
                  • <strong>Production forecasts</strong> — champion model selection and forecast
                  generation will use the new clusters. Forecast values will shift.
                </li>
                <li>
                  • <strong>Inventory planning</strong> — safety stock, EOQ, and exception detection
                  rely on cluster-based demand patterns
                </li>
              </ul>
              <p className="mt-2 text-[11px] text-amber-600 dark:text-amber-500">
                Recommendation: Re-run backtests after promoting to validate model accuracy with the
                new clusters.
              </p>
            </div>

            <p className="mt-3 text-xs text-muted-foreground">
              K={scenarioResult.result.optimal_k} clusters,{" "}
              {(
                scenarioResult.result.total_dfus ?? scenarioResult.result.total_skus
              )?.toLocaleString()}{" "}
              SKUs will be reassigned.
            </p>
